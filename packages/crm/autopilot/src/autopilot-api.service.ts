...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "POST /trigger/{triggerId}/contact/{contactId}": {
    "path": [
      "triggerId",
      "contactId"
    ],
  ...
  "GET /list/{listId}/contact/{contactId}": {
    "path": [
      "listId",
      "contactId"
    ],
  ...
  "POST /list/{listId}/contact/{contactId}": {
    "path": [
      "listId",
      "contactId"
    ],
  ...
  "DELETE /list/{listId}/contact/{contactId}": {
    "path": [
      "listId",
      "contactId"
    ],
...
export class AutopilotService {
  ...
  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('AUTOPILOT_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Autopilot API error (${response.status}): ${errorText}`)
      }
    }
