...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "GET /employees/{employeeId}/files/view": {
    "path": [
      "employeeId"
    ],
    "query": [],
  ...
  "GET /employees/{employeeId}/files/{fileId}": {
    "path": [
      "employeeId",
      "fileId"
    ],
  ...
  "POST /employees/{employeeId}/files/{fileId}": {
    "path": [
      "employeeId",
      "fileId"
    ],
...
export class BambooHrService {
  ...
  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('BAMBOO_HR_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`BambooHR API error (${response.status}): ${errorText}`)
      }
    }
