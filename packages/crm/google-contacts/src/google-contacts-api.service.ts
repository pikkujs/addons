...
export const GOOGLE_CONTACTS_OAUTH2_CONFIG = {
  tokenSecretId: 'GOOGLE_CONTACTS_TOKENS',
...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "PATCH /people/{contactId}:updateContact": {
    "path": [
      "contactId"
    ],
    "query": [
  ...
  "DELETE /people/{contactId}:deleteContact": {
    "path": [
      "contactId"
    ],
    "query": [],
...
export class GoogleContactsService {
  ...
  constructor(secrets: TypedSecretService, variables: TypedVariablesService) {
    this.baseUrl = variables.get('GOOGLE_CONTACTS_BASE_URL') as string
    this.oauth = new OAuth2Client(
      GOOGLE_CONTACTS_OAUTH2_CONFIG,
      'GOOGLE_CONTACTS_APP_CREDENTIALS',
      secrets
    )
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Google Contacts API error (${response.status}): ${errorText}`)
      }
    }
