...
export const GOOGLE_BIG_QUERY_OAUTH2_CONFIG = {
  tokenSecretId: 'GOOGLE_BIG_QUERY_TOKENS',
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Google BigQuery API error (${response.status}): ${errorText}`)
      }
    }
