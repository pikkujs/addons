...
export class GoogleFirebaseRealtimeDatabaseService {
  private baseUrl: string

  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('GOOGLE_FIREBASE_REALTIME_DATABASE_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Firebase Realtime DB API error (${response.status}): ${errorText}`)
      }
    }
