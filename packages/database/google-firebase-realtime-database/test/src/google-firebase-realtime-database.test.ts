...
test('google-firebase-realtime-database addon', async () => {
  const secrets = new LocalSecretService()
  // Set up secrets for the service
  // await secrets.setSecret('GOOGLE_FIREBASE_REALTIME_DATABASE_CREDENTIALS', { ... })
  ...
  try {
    const { passed, failed } = await rpc.invoke('testGoogleFirebaseRealtimeDatabase', {})
