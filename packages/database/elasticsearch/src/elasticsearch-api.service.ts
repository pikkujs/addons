...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "DELETE /{indexId}/_doc/{documentId}": {
    "path": [
      "indexId",
      "documentId"
    ],
  ...
  "POST /{indexId}/_update/{documentId}": {
    "path": [
      "indexId",
      "documentId"
    ],
...
export class ElasticsearchService {
  ...
  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('ELASTICSEARCH_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Elasticsearch API error (${response.status}): ${errorText}`)
      }
    }
