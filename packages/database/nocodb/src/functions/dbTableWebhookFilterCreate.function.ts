...
export const DbTableWebhookFilterCreateInput = z.object({
...
export const DbTableWebhookFilterCreateOutput = z.object({
  ...
  enabled: z.union([z.number().int(), z.boolean(), z.unknown()]).optional().describe("Whether this filter is enabled. Disabled filters are skipped during evaluation."),
  fk_level_id: z.union([z.string(), z.unknown()]).optional().describe("Foreign Key to List View Level"),
}).describe("Model for Filter")

export const dbTableWebhookFilterCreate = pikkuSessionlessFunc({
  description: "Create filter(s) in a given Hook",
  input: DbTableWebhookFilterCreateInput,
  output: DbTableWebhookFilterCreateOutput,
  errors: [BadRequestError],
  func: async ({ nocodb }, data) => {
    return nocodb.call("POST", "/api/v1/db/meta/hooks/{hookId}/filters", data) as any
  },
})
