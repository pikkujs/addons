...
export const DbTableFilterReadInput = z.object({
  viewId: z.string().describe("Unique View ID"),
  includeAllFilters: z.boolean().optional(),
  "xc-auth": z.string().describe("Auth Token is a JWT Token generated based on the logged-in user. By default, the token is only valid for 10 hours. However, you can change the value by defining it using environment variable NC_JWT_EXPIRES_IN."),
})

export const DbTableFilterReadOutput = z.object({
...
export const dbTableFilterRead = pikkuSessionlessFunc({
  description: "Get the filter data in a given View",
  input: DbTableFilterReadInput,
  output: DbTableFilterReadOutput,
  errors: [BadRequestError],
  func: async ({ nocodb }, data) => {
    return nocodb.call("GET", "/api/v1/db/meta/views/{viewId}/filters", data) as any
  },
})
