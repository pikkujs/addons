...
export const DbDataTableBulkAggregateInput = z.object({
...
export const DbDataTableBulkAggregateOutput = z.record(z.string(), z.unknown())

export const dbDataTableBulkAggregate = pikkuSessionlessFunc({
  description: "Read bulk aggregated data from a given table with given filters",
  input: DbDataTableBulkAggregateInput,
  output: DbDataTableBulkAggregateOutput,
  errors: [BadRequestError],
  func: async ({ nocodb }, data) => {
    return nocodb.call("POST", "/api/v2/tables/{tableId}/bulk/aggregate", data) as any
  },
})
