...
export const DbTableRowListInput = z.object({
  ...
  getHiddenColumns: z.string().optional().describe("Get hidden columns on List Api"),
  "xc-auth": z.string().describe("Auth Token is a JWT Token generated based on the logged-in user. By default, the token is only valid for 10 hours. However, you can change the value by defining it using environment variable NC_JWT_EXPIRES_IN."),
})

export const DbTableRowListOutput = z.object({
...
export const dbTableRowList = pikkuSessionlessFunc({
  description: "List all table rows in a given table and base",
  input: DbTableRowListInput,
  output: DbTableRowListOutput,
  errors: [BadRequestError],
  func: async ({ nocodb }, data) => {
    return nocodb.call("GET", "/api/v1/db/data/{orgs}/{baseName}/{tableName}", data) as any
  },
})
