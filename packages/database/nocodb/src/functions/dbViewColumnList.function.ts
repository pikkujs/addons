...
export const DbViewColumnListInput = z.object({
  viewId: z.string().describe("Unique View ID"),
  "xc-auth": z.string().describe("Auth Token is a JWT Token generated based on the logged-in user. By default, the token is only valid for 10 hours. However, you can change the value by defining it using environment variable NC_JWT_EXPIRES_IN."),
})

export const DbViewColumnListOutput = z.object({
  ...
  })).describe("List of column objects"),
  pageInfo: z.object({
...
}).describe("Model for Column List")

export const dbViewColumnList = pikkuSessionlessFunc({
  description: "List all columns by ViewID",
  input: DbViewColumnListInput,
  output: DbViewColumnListOutput,
  errors: [BadRequestError],
  func: async ({ nocodb }, data) => {
    return nocodb.call("GET", "/api/v1/db/meta/views/{viewId}/columns", data) as any
  },
})
