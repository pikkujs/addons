...
export const PluginWebhookListInput = z.object({
  "xc-auth": z.string().describe("Auth Token is a JWT Token generated based on the logged-in user. By default, the token is only valid for 10 hours. However, you can change the value by defining it using environment variable NC_JWT_EXPIRES_IN."),
})

export const PluginWebhookListOutput = z.object({
...
export const pluginWebhookList = pikkuSessionlessFunc({
  description: "List all webhook plugins",
  input: PluginWebhookListInput,
  output: PluginWebhookListOutput,
  errors: [BadRequestError],
  func: async ({ nocodb }, data) => {
    return nocodb.call("GET", "/api/v1/db/meta/plugins/webhook", data) as any
  },
})
