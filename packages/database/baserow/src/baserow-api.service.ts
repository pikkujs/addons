...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "POST /api/database/rows/table/{tableId}/batch/": {
    "path": [
      "tableId"
    ],
    "query": [],
  ...
  "PATCH /api/database/rows/table/{tableId}/batch/": {
    "path": [
      "tableId"
    ],
    "query": [],
  ...
  "POST /api/database/rows/table/{tableId}/batch-delete/": {
    "path": [
      "tableId"
    ],
    "query": [],
...
export class BaserowService {
  ...
  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('BASEROW_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Baserow API error (${response.status}): ${errorText}`)
      }
    }
