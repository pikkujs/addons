...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  "GET /databases/{database}/layouts/{layout}/records": {
    "path": [
      "database",
      "layout"
    ],
  ...
  "POST /databases/{database}/layouts/{layout}/records": {
    "path": [
      "database",
      "layout"
    ],
  ...
  "GET /databases/{database}/layouts/{layout}/records/{recordId}": {
    "path": [
      "database",
      "layout",
      "recordId"
  ...
  "POST /databases/{database}/layouts/{layout}/records/{recordId}": {
    "path": [
      "database",
      "layout",
      "recordId"
  ...
  "PATCH /databases/{database}/layouts/{layout}/records/{recordId}": {
    "path": [
      "database",
      "layout",
      "recordId"
  ...
  "DELETE /databases/{database}/layouts/{layout}/records/{recordId}": {
    "path": [
      "database",
      "layout",
      "recordId"
  ...
  "POST /databases/{database}/layouts/{layout}/_find": {
    "path": [
      "database",
      "layout"
    ],
  ...
  "GET /databases/{database}/layouts/{layout}/script/{scriptName}": {
    "path": [
      "database",
      "layout",
      "scriptName"
...
export class FilemakerService {
  ...
  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('FILEMAKER_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`FileMaker API error (${response.status}): ${errorText}`)
      }
    }
