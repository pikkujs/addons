...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "POST /outagedetection/v1/certificaterequests/renew": {
    "path": [],
    "query": [],
    "headers": []
  },
...
export class VenafiTlsProtectCloudService {
  private baseUrl: string

  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('VENAFI_TLS_PROTECT_CLOUD_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Venafi TLS Protect Cloud API error (${response.status}): ${errorText}`)
      }
    }
