...
test('venafi-tls-protect-cloud addon', async () => {
  const secrets = new LocalSecretService()
  // Set up secrets for the service
  // await secrets.setSecret('VENAFI_TLS_PROTECT_CLOUD_CREDENTIALS', { ... })
  ...
  try {
    const { passed, failed } = await rpc.invoke('testVenafiTlsProtectCloud', {})
