...
export class RaindropService {
  ...
  constructor(secrets: TypedSecretService, variables: TypedVariablesService) {
    this.baseUrl = variables.get('RAINDROP_BASE_URL') as string
    this.oauth = new OAuth2Client(
      RAINDROP_OAUTH2_CONFIG,
      'RAINDROP_APP_CREDENTIALS',
      secrets
    )
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Raindrop API error (${response.status}): ${errorText}`)
      }
    }
