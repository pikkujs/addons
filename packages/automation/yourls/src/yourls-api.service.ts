...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  "POST /yourls-api-shorten": {
    "path": [],
    "query": [],
    "headers": []
  },
  "POST /yourls-api-expand": {
    "path": [],
    "query": [],
    "headers": []
  },
  "GET /yourls-api-stats": {
    "path": [],
    "query": [
      "q"
    ],
...
export class YourlsService {
  private baseUrl: string

  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('YOURLS_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Yourls API error (${response.status}): ${errorText}`)
      }
    }
