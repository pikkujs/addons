...
export const GOOGLE_TASKS_OAUTH2_CONFIG = {
  tokenSecretId: 'GOOGLE_TASKS_TOKENS',
...
export class GoogleTasksService {
  ...
  constructor(secrets: TypedSecretService, variables: TypedVariablesService) {
    this.baseUrl = variables.get('GOOGLE_TASKS_BASE_URL') as string
    this.oauth = new OAuth2Client(
      GOOGLE_TASKS_OAUTH2_CONFIG,
      'GOOGLE_TASKS_APP_CREDENTIALS',
      secrets
    )
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Google Tasks API error (${response.status}): ${errorText}`)
      }
    }
