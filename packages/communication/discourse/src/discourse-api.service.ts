...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "DELETE /groups/{groupId}/members.json": {
    "path": [
      "groupId"
    ],
    "query": [],
...
export class DiscourseService {
  ...
  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('DISCOURSE_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Discourse API error (${response.status}): ${errorText}`)
      }
    }
