...
export const TWITTER_OAUTH2_CONFIG = {
  tokenSecretId: 'TWITTER_TOKENS',
...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "POST /dm_conversations/with/{participantId}/messages": {
    "path": [
      "participantId"
    ],
    "query": [],
...
export class TwitterService {
  ...
  constructor(secrets: TypedSecretService, variables: TypedVariablesService) {
    this.baseUrl = variables.get('TWITTER_BASE_URL') as string
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`X (Twitter) API error (${response.status}): ${errorText}`)
      }
    }
