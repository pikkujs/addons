...
export const CreateCustomObjectInput = z.object({
  custom_object: z.object({
  allows_attachments: z.boolean().optional().describe("Whether this object allows file attachments"),
  allows_photos: z.boolean().optional().describe("Whether this object allows photo uploads"),
  description: z.string().optional().describe("A description of the custom object"),
  include_in_list_view: z.boolean().optional().describe("Whether to include this object in list views"),
  key: z.string().optional().describe("Unique identifier. Writable on create only"),
  title: z.string().optional().describe("Display name for the object"),
  title_pluralized: z.string().optional().describe("Pluralized version of the object's title"),
}).optional(),
})

export const CreateCustomObjectOutput = z.object({
  custom_object: z.object({
...
export const createCustomObject = pikkuSessionlessFunc({
  ...
  output: CreateCustomObjectOutput,
  func: async ({ zendesk }, data) => {
    return zendesk.call("POST", "/api/v2/custom_objects", data) as any
  },
})
