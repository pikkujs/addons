...
export const ShowDynamicContentItemInput = z.object({
  dynamic_content_item_id: z.number().int().describe("The ID of the dynamic content item. Example: 47"),
})

export const ShowDynamicContentItemOutput = z.object({
  item: z.object({
...
export const showDynamicContentItem = pikkuSessionlessFunc({
  description: "#### Allowed For\n\n* Admins, Agents",
  input: ShowDynamicContentItemInput,
  output: ShowDynamicContentItemOutput,
  func: async ({ zendesk }, data) => {
    return zendesk.call("GET", "/api/v2/dynamic_content/items/{dynamic_content_item_id}", data) as any
  },
})
