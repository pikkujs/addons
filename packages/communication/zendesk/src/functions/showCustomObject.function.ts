...
export const ShowCustomObjectInput = z.object({
  custom_object_key: z.string().describe("The key of a custom object. Example: \"car\""),
  include_permissions_metadata: z.boolean().optional().describe("Include permission metadata in the response"),
  include_ui_path: z.boolean().optional().describe("Include UI path in the response"),
})

export const ShowCustomObjectOutput = z.object({
...
export const showCustomObject = pikkuSessionlessFunc({
  description: "Returns an object with the specified key\n#### Allowed For\n* Agents",
  input: ShowCustomObjectInput,
  output: ShowCustomObjectOutput,
  func: async ({ zendesk }, data) => {
    return zendesk.call("GET", "/api/v2/custom_objects/{custom_object_key}", data) as any
  },
})
