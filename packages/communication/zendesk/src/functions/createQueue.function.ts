...
export const CreateQueueOutput = z.object({
  queue: z.object({
...
export const createQueue = pikkuSessionlessFunc({
  description: "Creates a queue. Accepts a JSON queue definition as the request body.\n\n#### Allowed For\n\n* Admins",
  output: CreateQueueOutput,
  func: async ({ zendesk }) => {
    return zendesk.call("POST", "/api/v2/queues") as any
  },
})
