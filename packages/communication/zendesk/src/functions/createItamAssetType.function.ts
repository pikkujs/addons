...
export const CreateItamAssetTypeInput = z.object({
  asset_type: z.object({
...
export const CreateItamAssetTypeOutput = z.object({
  asset_type: z.object({
    created_at: z.string().datetime().optional().describe("The time the asset type was created"),
    created_by_user_id: z.number().int().optional().describe("The id of the user who created the asset type"),
    description: z.string().nullable().optional().describe("A description of the asset type"),
...
export const createItamAssetType = pikkuSessionlessFunc({
  description: "Creates an asset type.\n\n#### Allowed For\n\n* Admins",
  input: CreateItamAssetTypeInput,
  output: CreateItamAssetTypeOutput,
  func: async ({ zendesk }, data) => {
    return zendesk.call("POST", "/api/v2/it_asset_management/asset_types", data) as any
  },
})
