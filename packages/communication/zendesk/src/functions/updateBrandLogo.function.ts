...
export const UpdateBrandLogoInput = z.object({
  brand_id: z.number().int().describe("The ID of the brand. Example: 360002783572"),
  "brand[logo][uploaded_data]": z.string().describe("Brand logo image file"),
})

export const UpdateBrandLogoOutput = z.object({
  brand: z.object({
...
export const updateBrandLogo = pikkuSessionlessFunc({
  description: "Sets or replaces the logo on a brand using a multipart file upload.\n\n#### Allowed for\n* Admins",
  input: UpdateBrandLogoInput,
  output: UpdateBrandLogoOutput,
  func: async ({ zendesk }, data) => {
    return zendesk.call("PUT", "/api/v2/brands/{brand_id}/logo", data) as any
  },
})
