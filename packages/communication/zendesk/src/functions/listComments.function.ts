...
export const ListCommentsInput = z.object({
  ...
  since: z.string().optional().describe("Filters the comments from the given datetime"),
  role: z.string().optional().describe("One of \"agent\", \"end_user\". If not specified it does not filter"),
})
...
export const listComments = pikkuSessionlessFunc({
  ...
  func: async ({ zendesk }, data) => {
    return zendesk.call("GET", "/api/v2/requests/{request_id}/comments", data) as any
  },
})
