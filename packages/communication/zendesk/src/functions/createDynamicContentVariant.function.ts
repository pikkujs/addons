...
export const CreateDynamicContentVariantInput = z.object({
  dynamic_content_item_id: z.number().int().describe("The ID of the dynamic content item. Example: 47"),
})

export const CreateDynamicContentVariantOutput = z.object({
  variant: z.object({
...
export const createDynamicContentVariant = pikkuSessionlessFunc({
  ...
  input: CreateDynamicContentVariantInput,
  output: CreateDynamicContentVariantOutput,
  func: async ({ zendesk }, data) => {
    return zendesk.call("POST", "/api/v2/dynamic_content/items/{dynamic_content_item_id}/variants", data) as any
  },
})
