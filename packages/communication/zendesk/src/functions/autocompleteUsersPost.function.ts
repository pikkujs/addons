...
export const AutocompleteUsersPostInput = z.object({
  ...
  filter: z.string().optional().describe("Filter to apply to autocomplete results. Common values: `assignable`, `requester`.\n"),
  per_page: z.number().int().min(1).max(100).optional().describe("Number of results to return."),
  brand_id: z.union([z.literal("all"), z.number().int()]).optional().describe("When brand separation is enabled, scopes the autocomplete results to users\nbelonging to the specified brand. Only applicable when the account has brand separation enabled.\n\nAccepted values:\n\n* \"all\" — search all users across all brands (no brand filtering applied).\n* 0 — restrict the autocomplete results to account-scoped (brand-less) users only.\n* A numeric brand id — if the brand has user separation enabled, autocomplete\n  returns only end users belonging to that brand. Agents and admins are not included\n  in the results when filtering by a specific brand. If the brand does not have user\n  separation enabled, the request falls back to account scope (0).\n"),
  field_id: z.number().int().optional().describe("Field ID for lookup relationship autocomplete"),
  name: z.string().optional().describe("The name to search for the user"),
  phone: z.string().optional().describe("The phone number to search for the user"),
  source: z.string().optional().describe("Source for lookup relationship autocomplete"),
})

export const AutocompleteUsersPostOutput = z.object({
...
export const autocompleteUsersPost = pikkuSessionlessFunc({
  description: "Returns an array of users whose name starts with the value specified in the `name` property in the request body.\nIt only returns users with no foreign identities.\n\nThis endpoint accepts the same parameters as the [GET method](#autocomplete-users-by-query-string) but they are specified in the request body instead of the query string.\n\n#### Allowed For\n\n* Agents",
  input: AutocompleteUsersPostInput,
  output: AutocompleteUsersPostOutput,
  errors: [BadRequestError, InternalServerError],
  func: async ({ zendesk }, data) => {
    return zendesk.call("POST", "/api/v2/users/autocomplete", data) as any
  },
})
