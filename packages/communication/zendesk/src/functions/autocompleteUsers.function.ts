...
export const AutocompleteUsersInput = z.object({
  name: z.string().optional().describe("The name to search for the user. You must specify either `name` or `phone`.\n. Example: \"gil\""),
  phone: z.string().optional().describe("The phone number to search for the user. You must specify either `name` or `phone`.\n"),
  filter: z.enum(["assignable", "requester"]).optional().describe("Filter to apply to autocomplete results. Accepted values: `assignable`, `requester`.\n"),
...
export const AutocompleteUsersOutput = z.object({
...
export const autocompleteUsers = pikkuSessionlessFunc({
  ...
  input: AutocompleteUsersInput,
  output: AutocompleteUsersOutput,
  errors: [BadRequestError],
  func: async ({ zendesk }, data) => {
    return zendesk.call("GET", "/api/v2/users/autocomplete", data) as any
  },
})
