...
export const ShowManyUsersInput = z.object({
  ids: z.string().optional().describe("Accepts a comma-separated list of up to 100 user ids.\n. Example: \"1,2\""),
  external_ids: z.string().optional().describe("Accepts a comma-separated list of up to 100 external ids.\n. Example: \"abc,def\""),
  include_deleted: z.boolean().optional().describe("If true, returns inactive or deleted users.\n"),
  brand_id: z.union([z.literal("all"), z.number().int()]).optional().describe("When brand separation is enabled and `external_ids` is provided, scopes\nthe lookup to users belonging to the specified brand. Only applicable\nwhen the account has brand separation enabled.\n\nAccepted values:\n\n* \"all\" — return all matching users across all brands (no brand filtering applied).\n* 0 — restrict the lookup to account-scoped (brand-less) users only.\n* A numeric brand id — if the brand has user separation enabled, return\n  end users belonging to that brand plus all agents and admins (who are\n  account-level and always included). If the brand does not have user\n  separation enabled, the request falls back to account scope (0).\n\nWhen `ids` is provided instead of `external_ids`, brand filtering is not\napplied (user IDs are globally unique).\n. Example: \"all\""),
...
export const ShowManyUsersOutput = z.object({
...
export const showManyUsers = pikkuSessionlessFunc({
  description: "Accepts a comma-separated list of up to 100 user ids or external ids.\n\n#### Allowed For:\n\n* Agents",
  input: ShowManyUsersInput,
  output: ShowManyUsersOutput,
  func: async ({ zendesk }, data) => {
    return zendesk.call("GET", "/api/v2/users/show_many", data) as any
  },
})
