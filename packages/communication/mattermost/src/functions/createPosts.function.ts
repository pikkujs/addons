...
export const CreatePostsInput = z.object({
  ...
  root_id: z.string().optional().describe("The post ID to comment on"),
  file_ids: z.array(z.string()).optional().describe("A list of file IDs to associate with the post. Note that posts are limited to 5 files maximum. Please use additional posts for more files."),
  props: z.record(z.string(), z.unknown()).optional().describe("A general JSON property bag to attach to the post"),
})

export const CreatePostsOutput = z.object({
...
export const createPosts = pikkuSessionlessFunc({
  ...
  output: CreatePostsOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("POST", "/posts", data) as any
  },
})
