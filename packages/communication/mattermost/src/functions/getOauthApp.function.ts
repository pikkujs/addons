...
export const GetOauthAppInput = z.object({
  app_id: z.string().describe("Application client id"),
})

export const GetOauthAppOutput = z.object({
...
export const getOauthApp = pikkuSessionlessFunc({
  description: "Get an OAuth 2.0 client application registered with Mattermost.\n##### Permissions\nIf app creator, must have `mange_oauth` permission otherwise `manage_system_wide_oauth` permission is required.",
  input: GetOauthAppInput,
  output: GetOauthAppOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError, NotFoundError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("GET", "/oauth/apps/{app_id}", data) as any
  },
})
