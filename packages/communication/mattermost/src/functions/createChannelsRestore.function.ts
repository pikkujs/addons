...
export const CreateChannelsRestoreInput = z.object({
  channel_id: z.string().describe("Channel GUID"),
})

export const CreateChannelsRestoreOutput = z.object({
...
export const createChannelsRestore = pikkuSessionlessFunc({
  description: "Restore channel from the provided channel id string.\n\n__Minimum server version__: 3.10\n\n##### Permissions\n`manage_team` permission for the team of the channel.",
  input: CreateChannelsRestoreInput,
  output: CreateChannelsRestoreOutput,
  errors: [UnauthorizedError, ForbiddenError, NotFoundError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("POST", "/channels/{channel_id}/restore", data) as any
  },
})
