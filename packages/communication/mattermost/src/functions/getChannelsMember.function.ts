...
export const GetChannelsMemberInput = z.object({
  channel_id: z.string().describe("Channel GUID"),
  user_id: z.string().describe("User GUID"),
})

export const GetChannelsMemberOutput = z.object({
...
export const getChannelsMember = pikkuSessionlessFunc({
  description: "Get a channel member.\n##### Permissions\n`read_channel` permission for the channel.",
  input: GetChannelsMemberInput,
  output: GetChannelsMemberOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("GET", "/channels/{channel_id}/members/{user_id}", data) as any
  },
})
