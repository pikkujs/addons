...
export const CreateBotsDisableInput = z.object({
  bot_user_id: z.string().describe("Bot user ID"),
})

export const CreateBotsDisableOutput = z.object({
...
export const createBotsDisable = pikkuSessionlessFunc({
  description: "Disable a bot.\n##### Permissions\nMust have `manage_bots` permission. \n__Minimum server version__: 5.10",
  input: CreateBotsDisableInput,
  output: CreateBotsDisableOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("POST", "/bots/{bot_user_id}/disable", data) as any
  },
})
