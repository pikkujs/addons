...
export const DeleteUsersImageInput = z.object({
  user_id: z.string().describe("User GUID"),
})

export const DeleteUsersImageOutput = z.object({
  status: z.string().optional().describe("Will contain \"ok\" if the request was successful and there was nothing else to return"),
})

export const deleteUsersImage = pikkuSessionlessFunc({
  ...
  input: DeleteUsersImageInput,
  output: DeleteUsersImageOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError, NotFoundError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("DELETE", "/users/{user_id}/image", data) as any
  },
})
