...
export const GetHooksOutgoingInput = z.object({
  hook_id: z.string().describe("Outgoing webhook GUID"),
})

export const GetHooksOutgoingOutput = z.object({
...
export const getHooksOutgoing = pikkuSessionlessFunc({
  description: "Get an outgoing webhook given the hook id.\n##### Permissions\n`manage_webhooks` for system or `manage_webhooks` for the specific team or `manage_webhooks` for the channel.",
  input: GetHooksOutgoingInput,
  output: GetHooksOutgoingOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError, NotFoundError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("GET", "/hooks/outgoing/{hook_id}", data) as any
  },
})
