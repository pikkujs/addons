...
export const CreateUsersSearchInput = z.object({
  term: z.string().describe("The term to match against username, full name, nickname and email"),
  team_id: z.string().optional().describe("If provided, only search users on this team"),
  not_in_team_id: z.string().optional().describe("If provided, only search users not on this team"),
  in_channel_id: z.string().optional().describe("If provided, only search users in this channel"),
  not_in_channel_id: z.string().optional().describe("If provided, only search users not in this channel. Must specifiy `team_id` when using this option"),
  group_constrained: z.boolean().optional().describe("When used with `not_in_channel_id` or `not_in_team_id`, returns only the users that are allowed to join the channel or team based on its group constrains."),
  allow_inactive: z.boolean().optional().describe("When `true`, include deactivated users in the results"),
  without_team: z.boolean().optional().describe("Set this to `true` if you would like to search for users that are not on a team. This option takes precendence over `team_id`, `in_channel_id`, and `not_in_channel_id`."),
  limit: z.number().int().optional().default(100).describe("The maximum number of users to return in the results\n\n__Available as of server version 5.6. Defaults to `100` if not provided or on an earlier server version.__\n"),
})

export const CreateUsersSearchOutput = z.array(z.object({
...
export const createUsersSearch = pikkuSessionlessFunc({
  ...
  input: CreateUsersSearchInput,
  output: CreateUsersSearchOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("POST", "/users/search", data) as any
  },
})
