...
export const CreateChannelsDirectInput = z.object({
  body: z.array(z.string()).min(2).max(2),
})

export const CreateChannelsDirectOutput = z.object({
...
export const createChannelsDirect = pikkuSessionlessFunc({
  description: "Create a new direct message channel between two users.\n##### Permissions\nMust be one of the two users and have `create_direct_channel` permission. Having the `manage_system` permission voids the previous requirements.",
  input: CreateChannelsDirectInput,
  output: CreateChannelsDirectOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("POST", "/channels/direct", data) as any
  },
})
