...
export const CreateTeamsChannelsSearchInput = z.object({
  team_id: z.string().describe("Team GUID"),
  term: z.string().describe("The search term to match against the name or display name of channels"),
})

export const CreateTeamsChannelsSearchOutput = z.array(z.object({
...
export const createTeamsChannelsSearch = pikkuSessionlessFunc({
  description: "Search public channels on a team based on the search term provided in the request body.\n##### Permissions\nMust have the `list_team_channels` permission.",
  input: CreateTeamsChannelsSearchInput,
  output: CreateTeamsChannelsSearchOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError, NotFoundError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("POST", "/teams/{team_id}/channels/search", data) as any
  },
})
