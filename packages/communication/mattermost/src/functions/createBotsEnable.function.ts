...
export const CreateBotsEnableInput = z.object({
  bot_user_id: z.string().describe("Bot user ID"),
})

export const CreateBotsEnableOutput = z.object({
...
export const createBotsEnable = pikkuSessionlessFunc({
  description: "Enable a bot.\n##### Permissions\nMust have `manage_bots` permission. \n__Minimum server version__: 5.10",
  input: CreateBotsEnableInput,
  output: CreateBotsEnableOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("POST", "/bots/{bot_user_id}/enable", data) as any
  },
})
