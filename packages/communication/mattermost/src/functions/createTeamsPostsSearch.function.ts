...
export const CreateTeamsPostsSearchInput = z.object({
  team_id: z.string().describe("Team GUID"),
  terms: z.string().describe("The search terms as inputed by the user. To search for posts from a user include `from:someusername`, using a user's username. To search in a specific channel include `in:somechannel`, using the channel name (not the display name)."),
  is_or_search: z.boolean().describe("Set to true if an Or search should be performed vs an And search."),
  time_zone_offset: z.number().int().optional().default(0).describe("Offset from UTC of user timezone for date searches."),
  include_deleted_channels: z.boolean().optional().describe("Set to true if deleted channels should be included in the search. (archived channels)"),
  page: z.number().int().optional().default(0).describe("The page to select. (Only works with Elasticsearch)"),
  per_page: z.number().int().optional().default(60).describe("The number of posts per page. (Only works with Elasticsearch)"),
})

export const CreateTeamsPostsSearchOutput = z.object({
  ...
  matches: z.record(z.string(), z.array(z.string())).optional().describe("A mapping of post IDs to a list of matched terms within the post. This field will only be populated on servers running version 5.1 or greater with Elasticsearch enabled."),
})

export const createTeamsPostsSearch = pikkuSessionlessFunc({
  description: "Search posts in the team and from the provided terms string.\n##### Permissions\nMust be authenticated and have the `view_team` permission.",
  input: CreateTeamsPostsSearchInput,
  output: CreateTeamsPostsSearchOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("POST", "/teams/{team_id}/posts/search", data) as any
  },
})
