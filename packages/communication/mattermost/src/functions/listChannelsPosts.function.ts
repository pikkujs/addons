...
export const ListChannelsPostsInput = z.object({
  channel_id: z.string().describe("The channel ID to get the posts for"),
  page: z.string().optional().default("0").describe("The page to select"),
  per_page: z.string().optional().default("60").describe("The number of posts per page"),
  since: z.number().int().optional().describe("Provide a non-zero value in Unix time milliseconds to select posts created after that time"),
  before: z.string().optional().describe("A post id to select the posts that came before this one"),
  after: z.string().optional().describe("A post id to select the posts that came after this one"),
})

export const ListChannelsPostsOutput = z.object({
  ...
  next_post_id: z.string().optional().describe("The ID of next post. Not omitted when empty or not relevant."),
  prev_post_id: z.string().optional().describe("The ID of previous post. Not omitted when empty or not relevant."),
})

export const listChannelsPosts = pikkuSessionlessFunc({
  ...
  input: ListChannelsPostsInput,
  output: ListChannelsPostsOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("GET", "/channels/{channel_id}/posts", data) as any
  },
})
