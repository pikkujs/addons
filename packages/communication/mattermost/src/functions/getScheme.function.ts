...
export const GetSchemeInput = z.object({
  scheme_id: z.string().describe("Scheme GUID"),
})

export const GetSchemeOutput = z.object({
...
export const getScheme = pikkuSessionlessFunc({
  description: "Get a scheme from the provided scheme id.\n\n##### Permissions\nMust have `manage_system` permission.\n\n__Minimum server version__: 5.0",
  input: GetSchemeInput,
  output: GetSchemeOutput,
  errors: [UnauthorizedError, NotFoundError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("GET", "/schemes/{scheme_id}", data) as any
  },
})
