...
export const GetUsersUsernameInput = z.object({
  username: z.string().describe("Username"),
})

export const GetUsersUsernameOutput = z.object({
...
export const getUsersUsername = pikkuSessionlessFunc({
  description: "Get a user object by providing a username. Sensitive information will be sanitized out.\n##### Permissions\nRequires an active session but no other permissions.",
  input: GetUsersUsernameInput,
  output: GetUsersUsernameOutput,
  errors: [BadRequestError, UnauthorizedError, NotFoundError],
  func: async ({ mattermost }, data) => {
    return mattermost.call("GET", "/users/username/{username}", data) as any
  },
})
