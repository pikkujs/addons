...
export const ZOOM_OAUTH2_CONFIG = {
  tokenSecretId: 'ZOOM_TOKENS',
...
export class ZoomService {
  ...
  constructor(secrets: TypedSecretService, variables: TypedVariablesService) {
    this.baseUrl = variables.get('ZOOM_BASE_URL') as string
    this.oauth = new OAuth2Client(
      ZOOM_OAUTH2_CONFIG,
      'ZOOM_APP_CREDENTIALS',
      secrets
    )
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Zoom API error (${response.status}): ${errorText}`)
      }
    }
