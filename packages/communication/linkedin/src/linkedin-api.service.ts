...
export const LINKEDIN_OAUTH2_CONFIG = {
  tokenSecretId: 'LINKEDIN_TOKENS',
...
export class LinkedinService {
  ...
  constructor(secrets: TypedSecretService, variables: TypedVariablesService) {
    this.baseUrl = variables.get('LINKEDIN_BASE_URL') as string
    this.oauth = new OAuth2Client(
      LINKEDIN_OAUTH2_CONFIG,
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`LinkedIn API error (${response.status}): ${errorText}`)
      }
    }
