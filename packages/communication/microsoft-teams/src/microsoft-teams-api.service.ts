...
export const MICROSOFT_TEAMS_OAUTH2_CONFIG = {
  tokenSecretId: 'MICROSOFT_TEAMS_TOKENS',
...
export class MicrosoftTeamsService {
  ...
  constructor(secrets: TypedSecretService, variables: TypedVariablesService) {
    this.baseUrl = variables.get('MICROSOFT_TEAMS_BASE_URL') as string
    this.oauth = new OAuth2Client(
      MICROSOFT_TEAMS_OAUTH2_CONFIG,
      'MICROSOFT_TEAMS_APP_CREDENTIALS',
      secrets
    )
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Microsoft Teams API error (${response.status}): ${errorText}`)
      }
    }
