...
export const UsergroupsUsersUpdateInput = z.object({
  ...
  users: z.string().describe("A comma separated string of encoded user IDs that represent the entire list of users for the User Group."),
})

export const UsergroupsUsersUpdateOutput = z.object({
  ok: z.literal(true),
  usergroup: z.object({
...
}).describe("Schema for successful response from usergroups.users.update method")

export const usergroupsUsersUpdate = pikkuSessionlessFunc({
  description: "Update the list of users for a User Group",
  input: UsergroupsUsersUpdateInput,
  output: UsergroupsUsersUpdateOutput,
  func: async ({ slack }, data) => {
    return slack.call("POST", "/usergroups.users.update", data) as any
  },
})
