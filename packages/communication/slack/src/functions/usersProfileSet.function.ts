...
export const UsersProfileSetInput = z.object({
  token: z.string().describe("Authentication token. Requires scope: `users.profile:write`"),
  name: z.string().optional().describe("Name of a single key to set. Usable only if `profile` is not passed."),
  profile: z.string().optional().describe("Collection of key:value pairs presented as a URL-encoded JSON hash. At most 50 fields may be set. Each field name is limited to 255 characters."),
  user: z.string().optional().describe("ID of user to change. This argument may only be specified by team admins on paid teams."),
  value: z.string().optional().describe("Value to set a single key to. Usable only if `profile` is not passed."),
})

export const UsersProfileSetOutput = z.object({
  email_pending: z.string().email().optional(),
  ok: z.literal(true),
  profile: z.object({
  ...
  username: z.string(),
}).describe("Schema for successful response from users.profile.set method")

export const usersProfileSet = pikkuSessionlessFunc({
  description: "Set the profile information for a user.",
  input: UsersProfileSetInput,
  output: UsersProfileSetOutput,
  func: async ({ slack }, data) => {
    return slack.call("POST", "/users.profile.set", data) as any
  },
})
