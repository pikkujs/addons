...
export const GetIdsOfWorklogsDeletedSinceInput = z.object({
  since: z.number().int().optional().default(0).describe("The date and time, as a UNIX timestamp in milliseconds, after which deleted worklogs are returned."),
})

export const GetIdsOfWorklogsDeletedSinceOutput = z.object({
...
export const getIdsOfWorklogsDeletedSince = pikkuSessionlessFunc({
  ...
  input: GetIdsOfWorklogsDeletedSinceInput,
  output: GetIdsOfWorklogsDeletedSinceOutput,
  errors: [UnauthorizedError],
  func: async ({ jira }, data) => {
    return jira.call("GET", "/rest/api/3/worklog/deleted", data) as any
  },
})
