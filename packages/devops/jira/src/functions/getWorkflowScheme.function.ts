...
export const GetWorkflowSchemeInput = z.object({
  ...
  returnDraftIfExists: z.boolean().optional().default(false).describe("Returns the workflow scheme's draft rather than scheme itself, if set to true. If the workflow scheme does not have a draft, then the workflow scheme is returned."),
})

export const GetWorkflowSchemeOutput = z.object({
...
export const getWorkflowScheme = pikkuSessionlessFunc({
  description: "Returns a workflow scheme.\n\n**[Permissions](#permissions) required:** *Administer Jira* [global permission](https://confluence.atlassian.com/x/x4dKLg).",
  input: GetWorkflowSchemeInput,
  output: GetWorkflowSchemeOutput,
  errors: [UnauthorizedError, ForbiddenError, NotFoundError],
  func: async ({ jira }, data) => {
    return jira.call("GET", "/rest/api/3/workflowscheme/{id}", data) as any
  },
})
