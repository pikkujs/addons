...
export const UpdateCommentInput = z.object({
  ...
  notifyUsers: z.boolean().optional().default(true).describe("Whether users are notified when a comment is updated."),
  overrideEditableFlag: z.boolean().optional().default(false).describe("Whether screen security is overridden to enable uneditable fields to be edited. Available to Connect app users with the *Administer Jira* [global permission](https://confluence.atlassian.com/x/x4dKLg) and Forge apps acting on behalf of users with *Administer Jira* [global permission](https://confluence.atlassian.com/x/x4dKLg)."),
...
export const updateComment = pikkuSessionlessFunc({
  ...
  func: async ({ jira }, data) => {
    return jira.call("PUT", "/rest/api/3/issue/{issueIdOrKey}/comment/{id}", data) as any
  },
})
