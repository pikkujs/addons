...
export const CreateIssuesInput = z.object({
  issueUpdates: z.array(z.object({
...
export const CreateIssuesOutput = z.object({
  errors: z.array(z.object({
    elementErrors: z.object({
    ...
    status: z.number().int().optional(),
  })).optional().describe("Error details for failed issue creation requests."),
  ...
  })).optional().describe("Details of the issues created."),
}).describe("Details about the issues created and the errors for requests that failed.")

export const createIssues = pikkuSessionlessFunc({
  ...
  output: CreateIssuesOutput,
  errors: [BadRequestError, UnauthorizedError],
  func: async ({ jira }, data) => {
    return jira.call("POST", "/rest/api/3/issue/bulk", data) as any
  },
})
