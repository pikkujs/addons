...
export const GetVotesOutput = z.object({
  hasVoted: z.boolean().optional().describe("Whether the user making this request has voted on the issue."),
  self: z.string().url().optional().describe("The URL of these issue vote details."),
  ...
  })).optional().describe("List of the users who have voted on this issue. An empty list is returned when the calling user doesn't have the *View voters and watchers* project permission."),
  votes: z.number().int().optional().describe("The number of votes on the issue."),
}).describe("The details of votes on an issue.")

export const getVotes = pikkuSessionlessFunc({
  ...
  input: GetVotesInput,
  output: GetVotesOutput,
  errors: [UnauthorizedError, NotFoundError],
  func: async ({ jira }, data) => {
    return jira.call("GET", "/rest/api/3/issue/{issueIdOrKey}/votes", data) as any
  },
})
