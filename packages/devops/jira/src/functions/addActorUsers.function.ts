...
export const AddActorUsersInput = z.object({
  ...
  group: z.array(z.string()).optional().describe("The name of the group to add. This parameter cannot be used with the `groupId` parameter. As a group's name can change, use of `groupId` is recommended."),
  groupId: z.array(z.string()).optional().describe("The ID of the group to add. This parameter cannot be used with the `group` parameter."),
  user: z.array(z.string()).optional().describe("The user account ID of the user to add."),
})

export const AddActorUsersOutput = z.object({
...
export const addActorUsers = pikkuSessionlessFunc({
  ...
  input: AddActorUsersInput,
  output: AddActorUsersOutput,
  errors: [BadRequestError, UnauthorizedError, NotFoundError],
  func: async ({ jira }, data) => {
    return jira.call("POST", "/rest/api/3/project/{projectIdOrKey}/role/{id}", data) as any
  },
})
