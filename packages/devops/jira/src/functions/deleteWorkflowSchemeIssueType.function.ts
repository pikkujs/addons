...
export const DeleteWorkflowSchemeIssueTypeInput = z.object({
  id: z.number().int().describe("The ID of the workflow scheme."),
  issueType: z.string().describe("The ID of the issue type."),
  updateDraftIfNeeded: z.boolean().optional().describe("Set to true to create or update the draft of a workflow scheme and update the mapping in the draft, when the workflow scheme cannot be edited. Defaults to `false`."),
})

export const DeleteWorkflowSchemeIssueTypeOutput = z.object({
...
}).describe("Details about a workflow scheme.")

export const deleteWorkflowSchemeIssueType = pikkuSessionlessFunc({
  ...
  input: DeleteWorkflowSchemeIssueTypeInput,
  output: DeleteWorkflowSchemeIssueTypeOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError, NotFoundError],
  func: async ({ jira }, data) => {
    return jira.call("DELETE", "/rest/api/3/workflowscheme/{id}/issuetype/{issueType}", data) as any
  },
})
