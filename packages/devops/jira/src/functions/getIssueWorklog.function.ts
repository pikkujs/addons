...
export const GetIssueWorklogInput = z.object({
  ...
  startedAfter: z.number().int().optional().describe("The worklog start date and time, as a UNIX timestamp in milliseconds, after which worklogs are returned."),
  startedBefore: z.number().int().optional().describe("The worklog start date and time, as a UNIX timestamp in milliseconds, before which worklogs are returned."),
  expand: z.string().optional().default("").describe("Use [expand](#expansion) to include additional information about worklogs in the response. This parameter accepts`properties`, which returns worklog properties."),
})

export const GetIssueWorklogOutput = z.object({
  ...
  })).optional().describe("List of worklogs."),
}).describe("Paginated list of worklog details")

export const getIssueWorklog = pikkuSessionlessFunc({
  ...
  input: GetIssueWorklogInput,
  output: GetIssueWorklogOutput,
  errors: [UnauthorizedError, NotFoundError],
  func: async ({ jira }, data) => {
    return jira.call("GET", "/rest/api/3/issue/{issueIdOrKey}/worklog", data) as any
  },
})
