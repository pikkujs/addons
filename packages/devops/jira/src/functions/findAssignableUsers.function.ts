...
export const FindAssignableUsersInput = z.object({
  query: z.string().optional().describe("A query string that is matched against user attributes, such as `displayName`, and `emailAddress`, to find relevant users. The string can match the prefix of the attribute's value. For example, *query=john* matches a user with a `displayName` of *John Smith* and a user with an `emailAddress` of *johnson@example.com*. Required, unless `username` or `accountId` is specified."),
  sessionId: z.string().optional().describe("The sessionId of this request. SessionId is the same until the assignee is set."),
  ...
  project: z.string().optional().describe("The project ID or project key (case sensitive). Required, unless `issueKey` is specified."),
  issueKey: z.string().optional().describe("The key of the issue. Required, unless `project` is specified."),
  startAt: z.number().int().optional().default(0).describe("The index of the first item to return in a page of results (page offset)."),
  maxResults: z.number().int().optional().default(50).describe("The maximum number of items to return. This operation may return less than the maximum number of items even if more are available. The operation fetches users up to the maximum and then, from the fetched users, returns only the users that can be assigned to the issue."),
  actionDescriptorId: z.number().int().optional().describe("The ID of the transition."),
  recommend: z.boolean().optional().default(false),
})

export const FindAssignableUsersOutput = z.array(z.object({
...
export const findAssignableUsers = pikkuSessionlessFunc({
  ...
  input: FindAssignableUsersInput,
  output: FindAssignableUsersOutput,
  errors: [BadRequestError, UnauthorizedError, NotFoundError, TooManyRequestsError],
  func: async ({ jira }, data) => {
    return jira.call("GET", "/rest/api/3/user/assignable/search", data) as any
  },
})
