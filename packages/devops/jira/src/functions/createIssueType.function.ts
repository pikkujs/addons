...
export const CreateIssueTypeInput = z.object({
  ...
  hierarchyLevel: z.number().int().optional().describe("The hierarchy level of the issue type. Use:\n\n *  `-1` for Subtask.\n *  `0` for Base.\n\nDefaults to `0`."),
  name: z.string().describe("The unique name for the issue type. The maximum length is 60 characters."),
  type: z.enum(["subtask", "standard"]).optional().describe("Deprecated. Use `hierarchyLevel` instead. See the [deprecation notice](https://community.developer.atlassian.com/t/deprecation-of-the-epic-link-parent-link-and-other-related-fields-in-rest-apis-and-webhooks/54048) for details.\n\nWhether the issue type is `subtype` or `standard`. Defaults to `standard`."),
})

export const CreateIssueTypeOutput = z.object({
...
}).describe("Details about an issue type.")

export const createIssueType = pikkuSessionlessFunc({
  ...
  output: CreateIssueTypeOutput,
  errors: [BadRequestError, UnauthorizedError, ForbiddenError, ConflictError],
  func: async ({ jira }, data) => {
    return jira.call("POST", "/rest/api/3/issuetype", data) as any
  },
})
