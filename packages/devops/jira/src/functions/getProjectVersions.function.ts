...
export const GetProjectVersionsInput = z.object({
  projectIdOrKey: z.string().describe("The project ID or project key (case sensitive)."),
  expand: z.string().optional().describe("Use [expand](#expansion) to include additional information in the response. This parameter accepts `operations`, which returns actions that can be performed on the version."),
})

export const GetProjectVersionsOutput = z.array(z.object({
...
export const getProjectVersions = pikkuSessionlessFunc({
  ...
  output: GetProjectVersionsOutput,
  errors: [NotFoundError],
  func: async ({ jira }, data) => {
    return jira.call("GET", "/rest/api/3/project/{projectIdOrKey}/versions", data) as any
  },
})
