...
export const MoveVersionInput = z.object({
  id: z.string().describe("The ID of the version to be moved."),
  after: z.string().url().optional().describe("The URL (self link) of the version after which to place the moved version. Cannot be used with `position`."),
  position: z.enum(["Earlier", "Later", "First", "Last"]).optional().describe("An absolute position in which to place the moved version. Cannot be used with `after`."),
})

export const MoveVersionOutput = z.object({
...
export const moveVersion = pikkuSessionlessFunc({
  ...
  input: MoveVersionInput,
  output: MoveVersionOutput,
  errors: [BadRequestError, UnauthorizedError, NotFoundError],
  func: async ({ jira }, data) => {
    return jira.call("POST", "/rest/api/3/version/{id}/move", data) as any
  },
})
