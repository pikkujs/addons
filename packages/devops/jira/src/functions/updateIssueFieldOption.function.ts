...
export const UpdateIssueFieldOptionInput = z.object({
...
export const UpdateIssueFieldOptionOutput = z.object({
  config: z.object({
    attributes: z.array(z.enum(["notSelectable", "defaultValue"])).optional().describe("DEPRECATED"),
    scope: z.object({
      global: z.object({
...
export const updateIssueFieldOption = pikkuSessionlessFunc({
  ...
  input: UpdateIssueFieldOptionInput,
  output: UpdateIssueFieldOptionOutput,
  errors: [BadRequestError, ForbiddenError, NotFoundError],
  func: async ({ jira }, data) => {
    return jira.call("PUT", "/rest/api/3/field/{fieldKey}/option/{optionId}", data) as any
  },
})
