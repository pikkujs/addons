...
export const AddWorklogInput = z.object({
  ...
  reduceBy: z.string().optional().describe("The amount to reduce the issue's remaining estimate by, as days (\\#d), hours (\\#h), or minutes (\\#m). For example, *2d*. Required when `adjustEstimate` is `manual`."),
  expand: z.string().optional().default("").describe("Use [expand](#expansion) to include additional information about work logs in the response. This parameter accepts `properties`, which returns worklog properties."),
  overrideEditableFlag: z.boolean().optional().default(false).describe("Whether the worklog entry should be added to the issue even if the issue is not editable, because jira.issue.editable set to false or missing. For example, the issue is closed. Connect and Forge app users with *Administer Jira* [global permission](https://confluence.atlassian.com/x/x4dKLg) can use this flag."),
...
export const AddWorklogOutput = z.object({
  author: z.object({
...
export const addWorklog = pikkuSessionlessFunc({
  ...
  func: async ({ jira }, data) => {
    return jira.call("POST", "/rest/api/3/issue/{issueIdOrKey}/worklog", data) as any
  },
})
