...
export const GetVisibleIssueFieldOptionsInput = z.object({
  ...
  maxResults: z.number().int().optional().describe("The maximum number of items to return per page."),
  projectId: z.number().int().optional().describe("Filters the results to options that are only available in the specified project."),
})

export const GetVisibleIssueFieldOptionsOutput = z.object({
...
export const getVisibleIssueFieldOptions = pikkuSessionlessFunc({
  ...
  input: GetVisibleIssueFieldOptionsInput,
  output: GetVisibleIssueFieldOptionsOutput,
  errors: [UnauthorizedError, NotFoundError],
  func: async ({ jira }, data) => {
    return jira.call("GET", "/rest/api/3/field/{fieldKey}/option/suggestions/search", data) as any
  },
})
