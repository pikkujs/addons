...
export const UpdateWorklogInput = z.object({
  ...
  overrideEditableFlag: z.boolean().optional().default(false).describe("Whether the worklog should be added to the issue even if the issue is not editable. For example, because the issue is closed. Connect and Forge app users with *Administer Jira* [global permission](https://confluence.atlassian.com/x/x4dKLg) can use this flag."),
...
export const UpdateWorklogOutput = z.object({
  author: z.object({
...
export const updateWorklog = pikkuSessionlessFunc({
  ...
  output: UpdateWorklogOutput,
  errors: [BadRequestError, UnauthorizedError, NotFoundError],
  func: async ({ jira }, data) => {
    return jira.call("PUT", "/rest/api/3/issue/{issueIdOrKey}/worklog/{id}", data) as any
  },
})
