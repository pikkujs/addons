...
export const FindUsersWithBrowsePermissionInput = z.object({
  ...
  issueKey: z.string().optional().describe("The issue key for the issue. Required, unless `projectKey` is specified."),
  projectKey: z.string().optional().describe("The project key for the project (case sensitive). Required, unless `issueKey` is specified."),
...
export const FindUsersWithBrowsePermissionOutput = z.array(z.object({
...
export const findUsersWithBrowsePermission = pikkuSessionlessFunc({
  ...
  input: FindUsersWithBrowsePermissionInput,
  output: FindUsersWithBrowsePermissionOutput,
  errors: [BadRequestError, UnauthorizedError, NotFoundError, TooManyRequestsError],
  func: async ({ jira }, data) => {
    return jira.call("GET", "/rest/api/3/user/viewissue/search", data) as any
  },
})
