...
export const PullsUpdateReviewInput = z.object({
...
export const PullsUpdateReviewOutput = z.object({
  _links: z.object({
    html: z.object({
      href: z.string(),
    }),
...
export const pullsUpdateReview = pikkuSessionlessFunc({
  description: "Update the review summary comment with new text.",
  input: PullsUpdateReviewInput,
  output: PullsUpdateReviewOutput,
  errors: [UnprocessableContentError],
  func: async ({ github }, data) => {
    return github.call("PUT", "/repos/{owner}/{repo}/pulls/{pull_number}/reviews/{review_id}", data) as any
  },
})
