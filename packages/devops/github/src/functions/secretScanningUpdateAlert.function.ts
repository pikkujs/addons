...
export const SecretScanningUpdateAlertInput = z.object({
  ...
  resolution_comment: z.string().nullable().optional().describe("An optional comment when closing an alert. Cannot be updated or deleted. Must be `null` when changing `state` to `open`."),
...
export const SecretScanningUpdateAlertOutput = z.object({
...
export const secretScanningUpdateAlert = pikkuSessionlessFunc({
  ...
  input: SecretScanningUpdateAlertInput,
  output: SecretScanningUpdateAlertOutput,
  errors: [BadRequestError, NotFoundError, UnprocessableContentError],
  func: async ({ github }, data) => {
    return github.call("PATCH", "/repos/{owner}/{repo}/secret-scanning/alerts/{alert_number}", data) as any
  },
})
