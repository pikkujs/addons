...
export const ActionsUpdateRequiredWorkflowInput = z.object({
  org: z.string().describe("The organization name. The name is not case sensitive."),
  required_workflow_id: z.number().int().describe("The unique identifier of the required workflow."),
  repository_id: z.string().optional().describe("The ID of the repository that contains the workflow file."),
  scope: z.enum(["selected", "all"]).optional().default("all").describe("Enable the required workflow for all repositories or selected repositories in the organization."),
  selected_repository_ids: z.array(z.number().int()).optional().describe("A list of repository IDs where you want to enable the required workflow. A list of repository IDs where you want to enable the required workflow. You can only provide a list of repository ids when the `scope` is set to `selected`."),
  workflow_file_path: z.string().optional().describe("Path of the workflow file to be configured as a required workflow."),
})

export const ActionsUpdateRequiredWorkflowOutput = z.object({
  created_at: z.string().datetime(),
  id: z.number().describe("Unique identifier for a required workflow"),
  name: z.string().describe("Name present in the workflow file"),
  path: z.string().describe("Path of the workflow file"),
  ref: z.string().describe("Ref at which the workflow file will be selected"),
  repository: z.object({
  ...
  scope: z.enum(["all", "selected"]).describe("Scope of the required workflow"),
  selected_repositories_url: z.string().url().optional(),
  state: z.enum(["active", "deleted"]).describe("State of the required workflow"),
  updated_at: z.string().datetime(),
})

export const actionsUpdateRequiredWorkflow = pikkuSessionlessFunc({
  ...
  input: ActionsUpdateRequiredWorkflowInput,
  output: ActionsUpdateRequiredWorkflowOutput,
  errors: [UnprocessableContentError],
  func: async ({ github }, data) => {
    return github.call("PATCH", "/orgs/{org}/actions/required_workflows/{required_workflow_id}", data) as any
  },
})
