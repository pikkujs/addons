...
export const MigrationsStartForOrgInput = z.object({
  ...
  exclude: z.array(z.literal("repositories")).optional().describe("Exclude related items from being returned in the response in order to improve performance of the request. The array can include any of: `\"repositories\"`."),
  exclude_attachments: z.boolean().optional().default(false).describe("Indicates whether attachments should be excluded from the migration (to reduce migration archive file size)."),
  exclude_git_data: z.boolean().optional().default(false).describe("Indicates whether the repository git data should be excluded from the migration."),
  exclude_metadata: z.boolean().optional().default(false).describe("Indicates whether metadata should be excluded and only git source should be included for the migration."),
  exclude_owner_projects: z.boolean().optional().default(false).describe("Indicates whether projects owned by the organization or users should be excluded. from the migration."),
  exclude_releases: z.boolean().optional().default(false).describe("Indicates whether releases should be excluded from the migration (to reduce migration archive file size)."),
  lock_repositories: z.boolean().optional().default(false).describe("Indicates whether repositories should be locked (to prevent manipulation) while migrating data."),
  org_metadata_only: z.boolean().optional().default(false).describe("Indicates whether this should only include organization metadata (repositories array should be empty and will ignore other flags)."),
  repositories: z.array(z.string()).describe("A list of arrays indicating which repositories should be migrated."),
})

export const MigrationsStartForOrgOutput = z.object({
  ...
  })).describe("The repositories included in the migration. Only returned for export migrations."),
  state: z.string(),
  updated_at: z.string().datetime(),
  url: z.string().url(),
}).describe("A migration.")

export const migrationsStartForOrg = pikkuSessionlessFunc({
  description: "Initiates the generation of a migration archive.",
  input: MigrationsStartForOrgInput,
  output: MigrationsStartForOrgOutput,
  errors: [NotFoundError, UnprocessableContentError],
  func: async ({ github }, data) => {
    return github.call("POST", "/orgs/{org}/migrations", data) as any
  },
})
