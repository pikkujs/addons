...
export const ReposListForAuthenticatedUserInput = z.object({
  visibility: z.enum(["all", "public", "private"]).optional().default("all").describe("Limit results to repositories with the specified visibility."),
  affiliation: z.string().optional().default("owner,collaborator,organization_member").describe("Comma-separated list of values. Can include:  \n * `owner`: Repositories that are owned by the authenticated user.  \n * `collaborator`: Repositories that the user has been added to as a collaborator.  \n * `organization_member`: Repositories that the user has access to through being a member of an organization. This includes every repository on every team that the user is on."),
  type: z.enum(["all", "owner", "public", "private", "member"]).optional().default("all").describe("Limit results to repositories of the specified type. Will cause a `422` error if used in the same request as **visibility** or **affiliation**."),
  sort: z.enum(["created", "updated", "pushed", "full_name"]).optional().default("full_name").describe("The property to sort the results by."),
  ...
  before: z.string().datetime().optional().describe("Only show notifications updated before the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: `YYYY-MM-DDTHH:MM:SSZ`."),
})

export const ReposListForAuthenticatedUserOutput = z.array(z.object({
...
export const reposListForAuthenticatedUser = pikkuSessionlessFunc({
  ...
  input: ReposListForAuthenticatedUserInput,
  output: ReposListForAuthenticatedUserOutput,
  errors: [UnauthorizedError, ForbiddenError, UnprocessableContentError],
  func: async ({ github }, data) => {
    return github.call("GET", "/user/repos", data) as any
  },
})
