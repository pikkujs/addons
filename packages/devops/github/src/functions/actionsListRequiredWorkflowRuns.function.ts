...
export const ActionsListRequiredWorkflowRunsInput = z.object({
  ...
  status: z.enum(["completed", "action_required", "cancelled", "failure", "neutral", "skipped", "stale", "success", "timed_out", "in_progress", "queued", "requested", "waiting", "pending"]).optional().describe("Returns workflow runs with the check run `status` or `conclusion` that you specify. For example, a conclusion can be `success` or a status can be `in_progress`. Only GitHub can set a status of `waiting` or `requested`."),
  per_page: z.number().int().optional().default(30).describe("The number of results per page (max 100)."),
  page: z.number().int().optional().default(1).describe("Page number of the results to fetch."),
  created: z.string().datetime().optional().describe("Returns workflow runs created within the given date-time range. For more information on the syntax, see \"[Understanding the search syntax](https://docs.github.com/search-github/getting-started-with-searching-on-github/understanding-the-search-syntax#query-for-dates).\""),
  ...
  check_suite_id: z.number().int().optional().describe("Returns workflow runs with the `check_suite_id` that you specify."),
  head_sha: z.string().optional().describe("Only returns workflow runs that are associated with the specified `head_sha`."),
})

export const ActionsListRequiredWorkflowRunsOutput = z.object({
...
export const actionsListRequiredWorkflowRuns = pikkuSessionlessFunc({
  ...
  input: ActionsListRequiredWorkflowRunsInput,
  output: ActionsListRequiredWorkflowRunsOutput,
  func: async ({ github }, data) => {
    return github.call("GET", "/repos/{owner}/{repo}/actions/required_workflows/{required_workflow_id_for_repo}/runs", data) as any
  },
})
