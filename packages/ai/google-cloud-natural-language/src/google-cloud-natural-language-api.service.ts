...
export const GOOGLE_CLOUD_NATURAL_LANGUAGE_OAUTH2_CONFIG = {
  tokenSecretId: 'GOOGLE_CLOUD_NATURAL_LANGUAGE_TOKENS',
...
export class GoogleCloudNaturalLanguageService {
  private baseUrl: string
  private oauth: OAuth2Client

  constructor(secrets: TypedSecretService, variables: TypedVariablesService) {
    this.baseUrl = variables.get('GOOGLE_CLOUD_NATURAL_LANGUAGE_BASE_URL') as string
    this.oauth = new OAuth2Client(
      GOOGLE_CLOUD_NATURAL_LANGUAGE_OAUTH2_CONFIG,
      'GOOGLE_CLOUD_NATURAL_LANGUAGE_APP_CREDENTIALS',
      secrets
    )
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Google Cloud Natural Language API error (${response.status}): ${errorText}`)
      }
    }
