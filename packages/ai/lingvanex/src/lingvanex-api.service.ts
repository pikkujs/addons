...
export class LingvanexService {
  private baseUrl: string

  constructor(private creds: { token: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('LINGVANEX_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`LingvaNex API error (${response.status}): ${errorText}`)
      }
    }
