...
import { Answer, Question } from '../typesafe.types.js'
...
export const ask = pikkuSessionlessFunc({
  ...
  node: { displayName: 'Ask System One', category: 'AI', type: 'action' },
  input: AskInput,
  output: AskOutput,
  func: async ({ typesafe }, { state, questions }) => {
    return { answers: await typesafe.ask({ state, questions }) }
  },
})
