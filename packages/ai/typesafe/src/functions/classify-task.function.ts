...
import { SHAPES, taskQuestions } from '../task-questions.js'
...
export const classifyTask = pikkuSessionlessFunc({
  ...
  node: { displayName: 'Classify Task', category: 'AI', type: 'action' },
  input: ClassifyTaskInput,
  output: ClassifyTaskOutput,
  func: async ({ typesafe }, { task, context }) => {
    const answers = await typesafe.ask({
      state: { task, context },
