...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  "POST /products/mindee/expense_receipts/v4/predict": {
    "path": [],
    "query": [],
    "headers": []
  },
...
export class MindeeService {
  ...
  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('MINDEE_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Mindee API error (${response.status}): ${errorText}`)
      }
    }
