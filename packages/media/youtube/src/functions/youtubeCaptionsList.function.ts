...
export const YoutubeCaptionsListInput = z.object({
  ...
  id: z.array(z.string()).optional().describe("Returns the captions with the given IDs for Stubby or Apiary."),
  onBehalfOf: z.string().optional().describe("ID of the Google+ Page for the channel that the request is on behalf of."),
...
export const YoutubeCaptionsListOutput = z.object({
  ...
  })).optional().describe("A list of captions that match the request criteria."),
  kind: z.string().optional().default("youtube#captionListResponse").describe("Identifies what kind of resource this is. Value: the fixed string \"youtube#captionListResponse\"."),
  visitorId: z.string().optional().describe("The visitorId identifies the visitor."),
})

export const youtubeCaptionsList = pikkuSessionlessFunc({
  description: "Retrieves a list of resources, possibly filtered.",
  input: YoutubeCaptionsListInput,
  output: YoutubeCaptionsListOutput,
  func: async ({ youtube }, data) => {
    return youtube.call("GET", "/youtube/v3/captions", data) as any
  },
})
