...
export const YoutubeCommentsUpdateInput = z.object({
  ...
  part: z.array(z.string()).describe("The *part* parameter identifies the properties that the API response will include. You must at least include the snippet part in the parameter value since that part contains all of the properties that the API request can update."),
...
export const YoutubeCommentsUpdateOutput = z.object({
  etag: z.string().optional().describe("Etag of this resource."),
  id: z.string().optional().describe("The ID that YouTube uses to uniquely identify the comment."),
  kind: z.string().optional().default("youtube#comment").describe("Identifies what kind of resource this is. Value: the fixed string \"youtube#comment\"."),
  snippet: z.object({
...
}).describe("A *comment* represents a single YouTube comment.")

export const youtubeCommentsUpdate = pikkuSessionlessFunc({
  description: "Updates an existing resource.",
  input: YoutubeCommentsUpdateInput,
  output: YoutubeCommentsUpdateOutput,
  func: async ({ youtube }, data) => {
    return youtube.call("PUT", "/youtube/v3/comments", data) as any
  },
})
