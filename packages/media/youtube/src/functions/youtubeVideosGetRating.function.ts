...
export const YoutubeVideosGetRatingInput = z.object({
...
export const YoutubeVideosGetRatingOutput = z.object({
  ...
  items: z.array(z.object({
    rating: z.enum(["none", "like", "dislike"]).optional().describe("Rating of a video."),
    videoId: z.string().optional().describe("The ID that YouTube uses to uniquely identify the video."),
  })).optional().describe("A list of ratings that match the request criteria."),
  kind: z.string().optional().default("youtube#videoGetRatingResponse").describe("Identifies what kind of resource this is. Value: the fixed string \"youtube#videoGetRatingResponse\"."),
  visitorId: z.string().optional().describe("The visitorId identifies the visitor."),
})

export const youtubeVideosGetRating = pikkuSessionlessFunc({
  description: "Retrieves the ratings that the authorized user gave to a list of specified videos.",
  input: YoutubeVideosGetRatingInput,
  output: YoutubeVideosGetRatingOutput,
  func: async ({ youtube }, data) => {
    return youtube.call("GET", "/youtube/v3/videos/getRating", data) as any
  },
})
