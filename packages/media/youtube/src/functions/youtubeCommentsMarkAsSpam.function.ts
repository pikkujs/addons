...
export const YoutubeCommentsMarkAsSpamInput = z.object({
...
export const youtubeCommentsMarkAsSpam = pikkuSessionlessFunc({
  description: "Expresses the caller's opinion that one or more comments should be flagged as spam.",
  input: YoutubeCommentsMarkAsSpamInput,
  func: async ({ youtube }, data) => {
    return youtube.call("POST", "/youtube/v3/comments/markAsSpam", data)
  },
})
