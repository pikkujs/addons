...
export const YoutubeLiveStreamsUpdateInput = z.object({
...
export const YoutubeLiveStreamsUpdateOutput = z.object({
  cdn: z.object({
    format: z.string().optional().describe("The format of the video stream that you are sending to Youtube. "),
    frameRate: z.enum(["30fps", "60fps", "variable"]).optional().describe("The frame rate of the inbound video data."),
    ingestionInfo: z.object({
...
export const youtubeLiveStreamsUpdate = pikkuSessionlessFunc({
  description: "Updates an existing stream for the authenticated user.",
  input: YoutubeLiveStreamsUpdateInput,
  output: YoutubeLiveStreamsUpdateOutput,
  func: async ({ youtube }, data) => {
    return youtube.call("PUT", "/youtube/v3/liveStreams", data) as any
  },
})
