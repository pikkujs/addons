...
export const YoutubeCaptionsDownloadInput = z.object({
  ...
  tfmt: z.string().optional().describe("Convert the captions into this format. Supported options are sbv, srt, and vtt."),
  tlang: z.string().optional().describe("tlang is the language code; machine translate the captions into this language."),
})

export const youtubeCaptionsDownload = pikkuSessionlessFunc({
  description: "Downloads a caption track.",
  input: YoutubeCaptionsDownloadInput,
  func: async ({ youtube }, data) => {
    return youtube.call("GET", "/youtube/v3/captions/{id}", data)
  },
})
