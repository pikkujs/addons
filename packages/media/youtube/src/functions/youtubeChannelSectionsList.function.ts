...
export const YoutubeChannelSectionsListInput = z.object({
  ...
  channelId: z.string().optional().describe("Return the ChannelSections owned by the specified channel ID."),
  hl: z.string().optional().describe("Return content in specified language"),
  id: z.array(z.string()).optional().describe("Return the ChannelSections with the given IDs for Stubby or Apiary."),
  mine: z.boolean().optional().describe("Return the ChannelSections owned by the authenticated user."),
...
export const YoutubeChannelSectionsListOutput = z.object({
  ...
  })).optional().describe("A list of ChannelSections that match the request criteria."),
  kind: z.string().optional().default("youtube#channelSectionListResponse").describe("Identifies what kind of resource this is. Value: the fixed string \"youtube#channelSectionListResponse\"."),
  visitorId: z.string().optional().describe("The visitorId identifies the visitor."),
})

export const youtubeChannelSectionsList = pikkuSessionlessFunc({
  description: "Retrieves a list of resources, possibly filtered.",
  input: YoutubeChannelSectionsListInput,
  output: YoutubeChannelSectionsListOutput,
  func: async ({ youtube }, data) => {
    return youtube.call("GET", "/youtube/v3/channelSections", data) as any
  },
})
