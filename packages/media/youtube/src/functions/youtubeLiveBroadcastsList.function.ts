...
export const YoutubeLiveBroadcastsListInput = z.object({
  ...
  part: z.array(z.string()).describe("The *part* parameter specifies a comma-separated list of one or more liveBroadcast resource properties that the API response will include. The part names that you can include in the parameter value are id, snippet, contentDetails, status and statistics."),
  broadcastStatus: z.enum(["broadcastStatusFilterUnspecified", "all", "active", "upcoming", "completed"]).optional().describe("Return broadcasts with a certain status, e.g. active broadcasts."),
  broadcastType: z.enum(["broadcastTypeFilterUnspecified", "all", "event", "persistent"]).optional().describe("Return only broadcasts with the selected type."),
  id: z.array(z.string()).optional().describe("Return broadcasts with the given ids from Stubby or Apiary."),
...
export const YoutubeLiveBroadcastsListOutput = z.object({
  ...
  })).optional().describe("A list of broadcasts that match the request criteria."),
  kind: z.string().optional().default("youtube#liveBroadcastListResponse").describe("Identifies what kind of resource this is. Value: the fixed string \"youtube#liveBroadcastListResponse\"."),
...
export const youtubeLiveBroadcastsList = pikkuSessionlessFunc({
  description: "Retrieve the list of broadcasts associated with the given channel.",
  input: YoutubeLiveBroadcastsListInput,
  output: YoutubeLiveBroadcastsListOutput,
  func: async ({ youtube }, data) => {
    return youtube.call("GET", "/youtube/v3/liveBroadcasts", data) as any
  },
})
