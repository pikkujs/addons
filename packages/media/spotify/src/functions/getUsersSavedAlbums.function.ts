...
export const GetUsersSavedAlbumsInput = z.object({
...
export const GetUsersSavedAlbumsOutput = z.object({
  ...
  items: z.array(z.object({
    added_at: z.string().datetime().optional().describe("The date and time the album was saved\nTimestamps are returned in ISO 8601 format as Coordinated Universal Time (UTC) with a zero offset: YYYY-MM-DDTHH:MM:SSZ.\nIf the time is imprecise (for example, the date/time of an album release), an additional field indicates the precision; see for example, release_date in an album object.\n"),
    album: z.unknown().optional().describe("Information about the album."),
  })),
})

export const getUsersSavedAlbums = pikkuSessionlessFunc({
  description: "Get a list of the albums saved in the current Spotify user's 'Your Music' library.",
  input: GetUsersSavedAlbumsInput,
  output: GetUsersSavedAlbumsOutput,
  errors: [UnauthorizedError, ForbiddenError, TooManyRequestsError],
  func: async ({ spotify }, data) => {
    return spotify.call("GET", "/me/albums", data) as any
  },
})
