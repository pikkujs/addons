...
export const GetRecentlyPlayedInput = z.object({
  ...
  after: z.number().int().optional().describe("A Unix timestamp in milliseconds. Returns all items\nafter (but not including) this cursor position. If `after` is specified, `before`\nmust not be specified.\n"),
  before: z.number().int().optional().describe("A Unix timestamp in milliseconds. Returns all items\nbefore (but not including) this cursor position. If `before` is specified,\n`after` must not be specified.\n"),
})

export const GetRecentlyPlayedOutput = z.object({
  cursors: z.object({
  ...
  items: z.array(z.object({
    ...
    }).optional().describe("The context the track was played from."),
    played_at: z.string().datetime().optional().describe("The date and time the track was played."),
    track: z.object({
      ...
      uri: z.string().optional().describe("The [Spotify URI](/documentation/web-api/#spotify-uris-and-ids) for the track.\n"),
    }).optional().describe("The track the user listened to."),
  })).optional(),
})

export const getRecentlyPlayed = pikkuSessionlessFunc({
  description: "Get tracks from the current user's recently played tracks.\n_**Note**: Currently doesn't support podcast episodes._",
  input: GetRecentlyPlayedInput,
  output: GetRecentlyPlayedOutput,
  errors: [UnauthorizedError, ForbiddenError, TooManyRequestsError],
  func: async ({ spotify }, data) => {
    return spotify.call("GET", "/me/player/recently-played", data) as any
  },
})
