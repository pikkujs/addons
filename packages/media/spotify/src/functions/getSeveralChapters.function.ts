...
export const GetSeveralChaptersInput = z.object({
  ids: z.string().describe("A comma-separated list of the [Spotify IDs](/documentation/web-api/#spotify-uris-and-ids). For example: `ids=0IsXVP0JmcB2adSE338GkK,3ZXb8FKZGU0EHALYX6uCzU`. Maximum: 50 IDs.\n"),
...
export const GetSeveralChaptersOutput = z.object({
  chapters: z.array(z.object({
    audio_preview_url: z.string().describe("A URL to a 30 second preview (MP3 format) of the episode. `null` if not available.\n"),
    available_markets: z.array(z.string()).optional().describe("A list of the countries in which the chapter can be played, identified by their [ISO 3166-1 alpha-2](http://en.wikipedia.org/wiki/ISO_3166-1_alpha-2) code.\n"),
    chapter_number: z.number().int().describe("The number of the chapter\n"),
    ...
    audiobook: z.unknown().describe("The audiobook for which the chapter belongs.\n"),
  })),
})

export const getSeveralChapters = pikkuSessionlessFunc({
  description: "Get Spotify catalog information for several chapters identified by their Spotify IDs.<br />\n**Note: Chapters are only available for the US, UK, Ireland, New Zealand and Australia markets.**",
  input: GetSeveralChaptersInput,
  output: GetSeveralChaptersOutput,
  errors: [UnauthorizedError, ForbiddenError, TooManyRequestsError],
  func: async ({ spotify }, data) => {
    return spotify.call("GET", "/chapters", data) as any
  },
})
