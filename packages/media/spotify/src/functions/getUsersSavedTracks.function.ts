...
export const GetUsersSavedTracksInput = z.object({
...
export const GetUsersSavedTracksOutput = z.object({
  ...
  items: z.array(z.object({
    added_at: z.string().datetime().optional().describe("The date and time the track was saved.\nTimestamps are returned in ISO 8601 format as Coordinated Universal Time (UTC) with a zero offset: YYYY-MM-DDTHH:MM:SSZ.\nIf the time is imprecise (for example, the date/time of an album release), an additional field indicates the precision; see for example, release_date in an album object.\n"),
    track: z.object({
      ...
      uri: z.string().optional().describe("The [Spotify URI](/documentation/web-api/#spotify-uris-and-ids) for the track.\n"),
    }).optional().describe("Information about the track."),
  })),
})

export const getUsersSavedTracks = pikkuSessionlessFunc({
  description: "Get a list of the songs saved in the current Spotify user's 'Your Music' library.",
  input: GetUsersSavedTracksInput,
  output: GetUsersSavedTracksOutput,
  errors: [UnauthorizedError, ForbiddenError, TooManyRequestsError],
  func: async ({ spotify }, data) => {
    return spotify.call("GET", "/me/tracks", data) as any
  },
})
