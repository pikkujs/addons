...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "GET /sequences/{id}/subscriptions": {
    "path": [
      "id"
    ],
    "query": [
...
export class ConvertkitService {
  ...
  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('CONVERTKIT_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`ConvertKit API error (${response.status}): ${errorText}`)
      }
    }
