...
export const UserMailFolderChildFolderMessageDeltaInput = z.object({
  ...
  changeType: z.string().optional().describe("A custom query option to filter the delta response based on the type of change. Supported values are created, updated or deleted."),
...
export const UserMailFolderChildFolderMessageDeltaOutput = z.object({
  ...
  "@odata.deltaLink": z.string().nullable().optional(),
})

export const userMailFolderChildFolderMessageDelta = pikkuSessionlessFunc({
  ...
  input: UserMailFolderChildFolderMessageDeltaInput,
  output: UserMailFolderChildFolderMessageDeltaOutput,
  func: async ({ microsoftOutlook }, data) => {
    return microsoftOutlook.call("GET", "/users/{user-id}/mailFolders/{mailFolder-id}/childFolders/{mailFolder-id1}/messages/microsoft.graph.delta()", data) as any
  },
})
