...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "POST /contact-lists/{contactListId}/contacts": {
    "path": [
      "contactListId"
    ],
    "query": [],
...
export class EmeliaService {
  private baseUrl: string

  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('EMELIA_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Emelia API error (${response.status}): ${errorText}`)
      }
    }
