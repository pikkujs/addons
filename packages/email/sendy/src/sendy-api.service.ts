...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "POST /api/subscribers/active-subscriber-count.php": {
    "path": [],
    "query": [],
    "headers": []
  },
  ...
  "POST /api/subscribers/subscription-status.php": {
    "path": [],
    "query": [],
    "headers": []
  }
...
export class SendyService {
  ...
  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('SENDY_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Sendy API error (${response.status}): ${errorText}`)
      }
    }
