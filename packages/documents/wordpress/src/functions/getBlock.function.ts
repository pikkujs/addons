...
export const GetBlockInput = z.object({
  ...
  excerpt_length: z.string().optional().describe("Override the default excerpt length."),
  password: z.string().optional().describe("The password for the post if it is password protected."),
})

export const GetBlockOutput = z.object({
...
export const getBlock = pikkuSessionlessFunc({
  input: GetBlockInput,
  output: GetBlockOutput,
  func: async ({ wordpress }, data) => {
    return wordpress.call("GET", "/blocks/{id}", data) as any
  },
})
