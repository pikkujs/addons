...
export const UpdatePluginInput = z.object({
  plugin: z.string(),
  status: z.enum(["inactive", "active"]).optional().describe("The plugin activation status."),
})

export const UpdatePluginOutput = z.object({
...
export const updatePlugin = pikkuSessionlessFunc({
  input: UpdatePluginInput,
  output: UpdatePluginOutput,
  func: async ({ wordpress }, data) => {
    return wordpress.call("PUT", "/plugins/{plugin}", data) as any
  },
})
