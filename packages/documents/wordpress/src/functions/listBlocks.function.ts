...
export const ListBlocksInput = z.object({
  ...
  wp_pattern_category: z.union([z.array(z.number().int()), z.object({
...
})]).optional().describe("Limit result set to items with specific terms assigned in the wp_pattern_category taxonomy."),
  wp_pattern_category_exclude: z.union([z.array(z.number().int()), z.object({
  terms: z.array(z.number().int()).optional().default([]).describe("Term IDs."),
})]).optional().describe("Limit result set to items except those with specific terms assigned in the wp_pattern_category taxonomy."),
})

export const ListBlocksOutput = z.array(z.object({
...
export const listBlocks = pikkuSessionlessFunc({
  input: ListBlocksInput,
  output: ListBlocksOutput,
  func: async ({ wordpress }, data) => {
    return wordpress.call("GET", "/blocks", data) as any
  },
})
