...
export const ListPluginsInput = z.object({
  ...
  status: z.array(z.enum(["inactive", "active"])).optional().describe("Limits results to plugins with the given status."),
})

export const ListPluginsOutput = z.array(z.object({
...
export const listPlugins = pikkuSessionlessFunc({
  ...
  func: async ({ wordpress }, data) => {
    return wordpress.call("GET", "/plugins", data) as any
  },
})
