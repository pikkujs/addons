...
export const UpdatePlugin2Input = z.object({
  plugin: z.string(),
  status: z.enum(["inactive", "active"]).optional().describe("The plugin activation status."),
})

export const UpdatePlugin2Output = z.object({
...
export const updatePlugin2 = pikkuSessionlessFunc({
  input: UpdatePlugin2Input,
  output: UpdatePlugin2Output,
  func: async ({ wordpress }, data) => {
    return wordpress.call("PATCH", "/plugins/{plugin}", data) as any
  },
})
