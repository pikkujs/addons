...
export const ListTemplatePartsInput = z.object({
  ...
  wp_id: z.string().optional().describe("Limit to the specified post id."),
  area: z.string().optional().describe("Limit to the specified template part area."),
  post_type: z.string().optional().describe("Post type to get the templates for."),
})

export const ListTemplatePartsOutput = z.array(z.object({
...
export const listTemplateParts = pikkuSessionlessFunc({
  input: ListTemplatePartsInput,
  output: ListTemplatePartsOutput,
  func: async ({ wordpress }, data) => {
    return wordpress.call("GET", "/template-parts", data) as any
  },
})
