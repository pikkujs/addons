...
export const UpdateSettings2Input = z.object({
  ...
  use_smilies: z.boolean().optional().describe("Convert emoticons like :-) and :-P to graphics on display."),
  ...
  posts_per_page: z.number().int().optional().describe("Blog pages show at most."),
...
export const UpdateSettings2Output = z.object({
  title: z.string().optional().describe("Site title."),
  description: z.string().optional().describe("Site tagline."),
  url: z.string().url().optional().describe("Site URL."),
  email: z.string().email().optional().describe("This address is used for admin purposes, like new user notification."),
...
export const updateSettings2 = pikkuSessionlessFunc({
  input: UpdateSettings2Input,
  output: UpdateSettings2Output,
  func: async ({ wordpress }, data) => {
    return wordpress.call("PATCH", "/settings", data) as any
  },
})
