...
export const DeleteUsersMeInput = z.object({
  force: z.boolean().optional().default(false).describe("Required to be true, as users do not support trashing."),
  reassign: z.number().int().describe("Reassign the deleted user's posts and links to this user ID."),
})

export const DeleteUsersMeOutput = z.object({
...
export const deleteUsersMe = pikkuSessionlessFunc({
  input: DeleteUsersMeInput,
  output: DeleteUsersMeOutput,
  func: async ({ wordpress }, data) => {
    return wordpress.call("DELETE", "/users/me", data) as any
  },
})
