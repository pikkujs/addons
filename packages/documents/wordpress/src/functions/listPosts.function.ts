...
export const ListPostsInput = z.object({
  ...
  categories: z.union([z.array(z.number().int()), z.object({
  terms: z.array(z.number().int()).optional().default([]).describe("Term IDs."),
  include_children: z.boolean().optional().default(false).describe("Whether to include child terms in the terms limiting the result set."),
  operator: z.enum(["AND", "OR"]).optional().default("OR").describe("Whether items must be assigned all or any of the specified terms."),
})]).optional().describe("Limit result set to items with specific terms assigned in the categories taxonomy."),
  categories_exclude: z.union([z.array(z.number().int()), z.object({
  terms: z.array(z.number().int()).optional().default([]).describe("Term IDs."),
  include_children: z.boolean().optional().default(false).describe("Whether to include child terms in the terms limiting the result set."),
})]).optional().describe("Limit result set to items except those with specific terms assigned in the categories taxonomy."),
  tags: z.union([z.array(z.number().int()), z.object({
  terms: z.array(z.number().int()).optional().default([]).describe("Term IDs."),
  operator: z.enum(["AND", "OR"]).optional().default("OR").describe("Whether items must be assigned all or any of the specified terms."),
})]).optional().describe("Limit result set to items with specific terms assigned in the tags taxonomy."),
  tags_exclude: z.union([z.array(z.number().int()), z.object({
  terms: z.array(z.number().int()).optional().default([]).describe("Term IDs."),
})]).optional().describe("Limit result set to items except those with specific terms assigned in the tags taxonomy."),
  sticky: z.string().optional().describe("Limit result set to items that are sticky."),
  ignore_sticky: z.unknown().optional().default(true).describe("Whether to ignore sticky posts or not."),
  format: z.array(z.enum(["standard", "aside", "chat", "gallery", "link", "image", "quote", "status", "video", "audio"])).optional().describe("Limit result set to items assigned one or more given formats."),
})

export const ListPostsOutput = z.array(z.object({
...
export const listPosts = pikkuSessionlessFunc({
  input: ListPostsInput,
  output: ListPostsOutput,
  func: async ({ wordpress }, data) => {
    return wordpress.call("GET", "/posts", data) as any
  },
})
