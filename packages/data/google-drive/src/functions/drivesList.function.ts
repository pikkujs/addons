...
export const DrivesListInput = z.object({
  ...
  pageSize: z.number().int().min(1).max(100).optional().describe("Maximum number of shared drives to return per page."),
  ...
  useDomainAdminAccess: z.boolean().optional().describe("Issue the request as a domain administrator; if set to true, then all shared drives of the domain in which the requester is an administrator are returned."),
})

export const DrivesListOutput = z.object({
  ...
  })).optional().describe("The list of shared drives. If nextPageToken is populated, then this list may be incomplete and an additional page of results should be fetched."),
  kind: z.string().optional().default("drive#driveList").describe("Identifies what kind of resource this is. Value: the fixed string \"drive#driveList\"."),
  nextPageToken: z.string().optional().describe("The page token for the next page of shared drives. This will be absent if the end of the list has been reached. If the token is rejected for any reason, it should be discarded, and pagination should be restarted from the first page of results."),
}).describe("A list of shared drives.")

export const drivesList = pikkuSessionlessFunc({
  description: "Lists the user's shared drives.",
  input: DrivesListInput,
  output: DrivesListOutput,
  func: async ({ googleDrive }, data) => {
    return googleDrive.call("GET", "/drives", data) as any
  },
})
