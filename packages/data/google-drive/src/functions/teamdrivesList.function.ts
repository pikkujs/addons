...
export const TeamdrivesListInput = z.object({
  ...
  pageSize: z.number().int().min(1).max(100).optional().describe("Maximum number of Team Drives to return."),
  pageToken: z.string().optional().describe("Page token for Team Drives."),
  q: z.string().optional().describe("Query string for searching Team Drives."),
  useDomainAdminAccess: z.boolean().optional().describe("Issue the request as a domain administrator; if set to true, then all Team Drives of the domain in which the requester is an administrator are returned."),
})

export const TeamdrivesListOutput = z.object({
  kind: z.string().optional().default("drive#teamDriveList").describe("Identifies what kind of resource this is. Value: the fixed string \"drive#teamDriveList\"."),
  nextPageToken: z.string().optional().describe("The page token for the next page of Team Drives. This will be absent if the end of the Team Drives list has been reached. If the token is rejected for any reason, it should be discarded, and pagination should be restarted from the first page of results."),
  teamDrives: z.array(z.object({
  ...
  })).optional().describe("The list of Team Drives. If nextPageToken is populated, then this list may be incomplete and an additional page of results should be fetched."),
}).describe("A list of Team Drives.")

export const teamdrivesList = pikkuSessionlessFunc({
  description: "Deprecated use drives.list instead.",
  input: TeamdrivesListInput,
  output: TeamdrivesListOutput,
  func: async ({ googleDrive }, data) => {
    return googleDrive.call("GET", "/teamdrives", data) as any
  },
})
