...
export const PermissionsCreateInput = z.object({
  ...
  emailMessage: z.string().optional().describe("A plain text custom message to include in the notification email."),
  enforceSingleParent: z.boolean().optional().describe("Deprecated. See moveToNewOwnersRoot for details."),
  moveToNewOwnersRoot: z.boolean().optional().describe("This parameter will only take effect if the item is not in a shared drive and the request is attempting to transfer the ownership of the item. If set to true, the item will be moved to the new owner's My Drive root folder and all prior parents removed. If set to false, parents are not changed."),
  sendNotificationEmail: z.boolean().optional().describe("Whether to send a notification email when sharing to users or groups. This defaults to true for users and groups, and is not allowed for other requests. It must not be disabled for ownership transfers."),
  ...
  transferOwnership: z.boolean().optional().describe("Whether to transfer ownership to the specified user and downgrade the current owner to a writer. This parameter is required as an acknowledgement of the side effect. File owners can only transfer ownership of files existing on My Drive. Files existing in a shared drive are owned by the organization that owns that shared drive. Ownership transfers are not supported for files and folders in shared drives. Organizers of a shared drive can move items from that shared drive into their My Drive which transfers the ownership to them."),
...
export const PermissionsCreateOutput = z.object({
  allowFileDiscovery: z.boolean().optional().describe("Whether the permission allows the file to be discovered through search. This is only applicable for permissions of type domain or anyone."),
  deleted: z.boolean().optional().describe("Whether the account associated with this permission has been deleted. This field only pertains to user and group permissions."),
  displayName: z.string().optional().describe("The \"pretty\" name of the value of the permission. The following is a list of examples for each type of permission:  \n- user - User's full name, as defined for their Google Account, such as \"Joe Smith.\" \n- group - Name of the Google Group, such as \"The Company Administrators.\" \n- domain - String domain name, such as \"your-company.com.\" \n- anyone - No displayName is present."),
  domain: z.string().optional().describe("The domain to which this permission refers. The following options are currently allowed:  \n- The entire domain, such as \"your-company.com.\" \n- A target audience, such as \"ID.audience.googledomains.com.\""),
  ...
  type: z.string().optional().describe("The type of the grantee. Valid values are:  \n- user \n- group \n- domain \n- anyone  When creating a permission, if type is user or group, you must provide an emailAddress for the user or group. When type is domain, you must provide a domain. There isn't extra information required for the anyone type."),
  view: z.string().optional().describe("Indicates the view for this permission. Only populated for permissions that belong to a view. published is the only supported value."),
}).describe("A permission for a file. A permission grants a user, group, domain, or the world access to a file or a folder hierarchy.")

export const permissionsCreate = pikkuSessionlessFunc({
  ...
  output: PermissionsCreateOutput,
  func: async ({ googleDrive }, data) => {
    return googleDrive.call("POST", "/files/{fileId}/permissions", data) as any
  },
})
