...
export const RevisionsGetInput = z.object({
  ...
  acknowledgeAbuse: z.boolean().optional().describe("Whether the user is acknowledging the risk of downloading known malware or other abusive files. This is only applicable when alt=media."),
})

export const RevisionsGetOutput = z.object({
...
export const revisionsGet = pikkuSessionlessFunc({
  description: "Gets a revision's metadata or content by ID.",
  input: RevisionsGetInput,
  output: RevisionsGetOutput,
  func: async ({ googleDrive }, data) => {
    return googleDrive.call("GET", "/files/{fileId}/revisions/{revisionId}", data) as any
  },
})
