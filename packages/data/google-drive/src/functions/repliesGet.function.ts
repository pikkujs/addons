...
export const RepliesGetInput = z.object({
  ...
  includeDeleted: z.boolean().optional().describe("Whether to return deleted replies. Deleted replies will not include their original content."),
})

export const RepliesGetOutput = z.object({
...
export const repliesGet = pikkuSessionlessFunc({
  description: "Gets a reply by ID.",
  input: RepliesGetInput,
  output: RepliesGetOutput,
  func: async ({ googleDrive }, data) => {
    return googleDrive.call("GET", "/files/{fileId}/comments/{commentId}/replies/{replyId}", data) as any
  },
})
