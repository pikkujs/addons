...
export const RetrieveAPageInput = z.object({
  page_id: z.string().describe("The ID of the page to retrieve."),
  filter_properties: z.array(z.string()).max(100).optional().describe("Supply a list of property IDs to filter properties in the response. Note that if a page doesn't have a property, it won't be included in the filtered response."),
...
export const RetrieveAPageOutput = z.union([z.object({
...
export const retrieveAPage = pikkuSessionlessFunc({
  description: "Retrieve a page",
  input: RetrieveAPageInput,
  output: RetrieveAPageOutput,
  ...
  func: async ({ notion }, data) => {
    return notion.call("GET", "/v1/pages/{page_id}", data) as any
  },
})
