...
export const UpdateADataSourceInput = z.object({
...
})).max(100).optional().describe("Title of data source as it appears in Notion."),
...
})]), z.unknown()]).optional().describe("Page icon."),
  properties: z.record(z.string(), z.union([z.object({
  name: z.string().optional().describe("The name of the property."),
...
}), z.unknown()])).optional().describe("The property schema of the data source. The keys are property names or IDs, and the values are property configuration objects. Properties set to null will be removed."),
  ...
  parent: z.object({
  type: z.string().optional().describe("Always `database_id`"),
  database_id: z.string().describe("The ID of the parent database (with or without dashes), for example, 195de9221179449fab8075a27c979105"),
}).optional().describe("The parent of the data source, when moving it to a different database. If not provided, the parent will not be updated."),
})

export const UpdateADataSourceOutput = z.union([z.object({
...
export const updateADataSource = pikkuSessionlessFunc({
  description: "Update a data source",
  input: UpdateADataSourceInput,
  output: UpdateADataSourceOutput,
  ...
  func: async ({ notion }, data) => {
    return notion.call("PATCH", "/v1/data_sources/{data_source_id}", data) as any
  },
})
