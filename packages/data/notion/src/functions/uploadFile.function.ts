...
export const UploadFileInput = z.object({
  ...
  file: z.record(z.string(), z.unknown()).describe("The raw binary file contents to upload."),
  part_number: z.string().optional().describe("When uploading files greater than 20MB in parts, this is the current part number. Must be an integer between 1 and 1,000."),
})
...
export const uploadFile = pikkuSessionlessFunc({
  ...
  func: async ({ notion }, data) => {
    return notion.call("POST", "/v1/file_uploads/{file_upload_id}/send", data) as any
  },
})
