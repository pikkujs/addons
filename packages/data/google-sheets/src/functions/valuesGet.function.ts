...
export const ValuesGetInput = z.object({
  spreadsheetId: z.string().describe("The ID of the spreadsheet to retrieve data from."),
  range: z.string().describe("The [A1 notation or R1C1 notation](/sheets/api/guides/concepts#cell) of the range to retrieve values from."),
...
export const ValuesGetOutput = z.object({
...
}).describe("Data within a range of the spreadsheet.")

export const valuesGet = pikkuSessionlessFunc({
  ...
  output: ValuesGetOutput,
  func: async ({ googleSheets }, data) => {
    return googleSheets.call("GET", "/v4/spreadsheets/{spreadsheetId}/values/{range}", data) as any
  },
})
