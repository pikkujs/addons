...
export const DocsDocumentsGetInput = z.object({
  ...
  suggestionsViewMode: z.enum(["DEFAULT_FOR_CURRENT_ACCESS", "SUGGESTIONS_INLINE", "PREVIEW_SUGGESTIONS_ACCEPTED", "PREVIEW_WITHOUT_SUGGESTIONS"]).optional().describe("The suggestions view mode to apply to the document. This allows viewing the document with all suggestions inline, accepted or rejected. If one is not specified, DEFAULT_FOR_CURRENT_ACCESS is used."),
})

export const DocsDocumentsGetOutput = z.any()

export const docsDocumentsGet = pikkuSessionlessFunc({
  description: "Gets the latest version of the specified document.",
  input: DocsDocumentsGetInput,
  output: DocsDocumentsGetOutput,
  func: async ({ googleDocs }, data) => {
    return googleDocs.call("GET", "/v1/documents/{documentId}", data) as any
  },
})
