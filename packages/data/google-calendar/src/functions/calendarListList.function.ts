...
export const CalendarListListInput = z.object({
  ...
  minAccessRole: z.enum(["freeBusyReader", "owner", "reader", "writer"]).optional().describe("The minimum access role for the user in the returned entries. Optional. The default is no restriction."),
...
export const CalendarListListOutput = z.object({
  ...
  })).optional().describe("Calendars that are present on the user's calendar list."),
  kind: z.string().optional().default("calendar#calendarList").describe("Type of the collection (\"calendar#calendarList\")."),
...
export const calendarListList = pikkuSessionlessFunc({
  description: "Returns the calendars on the user's calendar list.",
  input: CalendarListListInput,
  output: CalendarListListOutput,
  func: async ({ googleCalendar }, data) => {
    return googleCalendar.call("GET", "/users/me/calendarList", data) as any
  },
})
