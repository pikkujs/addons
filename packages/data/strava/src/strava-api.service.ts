...
export const STRAVA_OAUTH2_CONFIG = {
  tokenSecretId: 'STRAVA_TOKENS',
...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "GET /activities/{activityId}/kudos": {
    "path": [
      "activityId"
    ],
    "query": [],
  ...
  "GET /activities/{activityId}/zones": {
    "path": [
      "activityId"
    ],
    "query": [],
...
export class StravaService {
  ...
  constructor(secrets: TypedSecretService, variables: TypedVariablesService) {
    this.baseUrl = variables.get('STRAVA_BASE_URL') as string
    this.oauth = new OAuth2Client(
      STRAVA_OAUTH2_CONFIG,
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Strava API error (${response.status}): ${errorText}`)
      }
    }
