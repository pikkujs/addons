...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "GET /v3/profiles/{profileId}/borderless-accounts/{borderlessAccountId}/statement.json": {
    "path": [
      "profileId",
      "borderlessAccountId"
    ],
...
export class WiseService {
  ...
  constructor(private creds: { token: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('WISE_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`Wise API error (${response.status}): ${errorText}`)
      }
    }
