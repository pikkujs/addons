...
export const CustomerCardAddOutput = z.object({
  ...
  expMonth: z.number().describe('Two-digit number representing the card\'s expiration month'),
  expYear: z.number().describe('Four-digit number representing the card\'s expiration year'),
...
export const customerCardAdd = pikkuSessionlessFunc({
  ...
  func: async ({ stripe }, { customerId, source, metadata }) => {
    const result = await stripe.customers.createSource(customerId, {
      source,
      ...(metadata ? { metadata } : {}),
    })
    return CustomerCardAddOutput.parse(fromStripeObject(result))
  },
})
