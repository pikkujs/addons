...
export const ChargeUpdateInput = z.object({
  ...
  receiptEmail: z.string().optional().describe('The email address to which this charge\'s receipt will be sent'),
  ...
  fraudDetails: z.object({
    userReport: z.enum(['fraudulent', 'safe']).describe('User-provided fraud report for this charge'),
...
export const chargeUpdate = pikkuSessionlessFunc({
  ...
  func: async ({ stripe }, { chargeId, ...data }) => {
    const result = await stripe.charges.update(chargeId, toStripeParams(data))
    const camel = fromStripeObject(result)
    return ChargeUpdateOutput.parse({ ...camel, created: epochToIso(result.created) })
  },
})
