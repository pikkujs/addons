...
export const ShippingSchema = z.object({
  ...
  trackingNumber: z.string().optional().describe('The tracking number for a physical product, obtained from the delivery service'),
})
...
export const ChargeCreateInput = z.object({
  ...
  receiptEmail: z.string().optional().describe('The email address to which this charge\'s receipt will be sent. The receipt will not be sent until the charge is paid'),
  ...
  statementDescriptor: z.string().optional().describe('For a non-card charge, text that appears on the customer\'s statement as the statement descriptor'),
  statementDescriptorSuffix: z.string().optional().describe('Provides information about a card charge. Concatenated to the account\'s statement descriptor prefix'),
})
...
export const chargeCreate = pikkuSessionlessFunc({
  ...
  func: async ({ stripe }, data) => {
    const result = await stripe.charges.create(toStripeParams(data))
    const camel = fromStripeObject(result)
    return ChargeCreateOutput.parse({ ...camel, created: epochToIso(result.created) })
  },
})
