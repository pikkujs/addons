...
export const BillingPortalSessionCreateInput = z.object({
  ...
  returnUrl: z.string().describe('URL the customer is redirected to when they leave the portal'),
})
...
export const BillingPortalSessionCreateOutput = z.object({
  ...
  returnUrl: z.string().nullable().describe('The configured return URL'),
})
...
export const billingPortalSessionCreate = pikkuSessionlessFunc({
  ...
  func: async ({ stripe }, { customer, returnUrl }) => {
    const result = await stripe.billingPortal.sessions.create({
      customer,
      return_url: returnUrl,
    })
    return BillingPortalSessionCreateOutput.parse({
      id: result.id,
