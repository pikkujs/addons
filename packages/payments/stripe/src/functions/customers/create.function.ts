...
export const CustomerCreateInput = z.object({
  ...
  paymentMethod: z.string().optional().describe('The ID of a payment method to attach to the customer'),
...
export const customerCreate = pikkuSessionlessFunc({
  ...
  func: async ({ stripe }, data) => {
    const result = await stripe.customers.create(toStripeParams(data))
    const camel = fromStripeObject(result)
    return CustomerCreateOutput.parse({ ...camel, created: epochToIso(result.created) })
  },
})
