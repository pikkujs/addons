...
export const CustomerUpdateInput = z.object({
  ...
  defaultSource: z.string().optional().describe('ID of the default payment source for the customer'),
  preferredLocales: z.array(z.string()).optional().describe('Customer\'s preferred languages, ordered by preference'),
})
...
export const customerUpdate = pikkuSessionlessFunc({
  ...
  func: async ({ stripe }, { customerId, ...data }) => {
    const result = await stripe.customers.update(customerId, toStripeParams(data))
    const camel = fromStripeObject(result)
    return CustomerUpdateOutput.parse({ ...camel, created: epochToIso(result.created) })
  },
})
