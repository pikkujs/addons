...
export const SharingListFileMembersInput = z.object({
  ...
  limit: z.number().optional().describe("Number of members to return max per query. Defaults to 100 if no limit is specified."),
  actions: z.array(z.object({
  ".tag": z.enum(["leave_a_copy", "make_editor", "make_owner", "make_viewer", "make_viewer_no_comment", "remove", "other"]).optional(),
})).optional().describe("The actions for which to return permissions on a member."),
  file: z.string().optional().describe("The file for which you want to see members."),
})

export const SharingListFileMembersOutput = z.object({
