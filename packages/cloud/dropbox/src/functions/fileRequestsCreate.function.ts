...
export const FileRequestsCreateInput = z.object({
  ...
  destination: z.string().optional().describe("The path of the folder in the Dropbox where uploaded files will be sent. For apps with the app folder permission, this will be relative to the app folder."),
  deadline: z.object({
  ...
  title: z.string().optional().describe("The title of the file request. Must not be empty."),
})

export const FileRequestsCreateOutput = z.object({
...
export const fileRequestsCreate = pikkuSessionlessFunc({
  description: "Creates a file request for this user.",
  input: FileRequestsCreateInput,
  output: FileRequestsCreateOutput,
  func: async ({ dropbox }, data) => {
    return dropbox.call("POST", "/file_requests/create", data) as any
  },
})
