...
export const FilesGetThumbnailBatchInput = z.object({
...
})).optional().describe("List of files to get thumbnails."),
})

export const FilesGetThumbnailBatchOutput = z.object({
  entries: z.array(z.object({
    failure: z.object({
      path: z.object({
    ...
    }).optional().describe("metadata: None\nthumbnail: None\n"),
  })).optional().describe("List of files and their thumbnails."),
}).describe("entries: List of files and their thumbnails.\n")

export const filesGetThumbnailBatch = pikkuSessionlessFunc({
  ...
  input: FilesGetThumbnailBatchInput,
  output: FilesGetThumbnailBatchOutput,
  func: async ({ dropbox }, data) => {
    return dropbox.call("POST", "/files/get_thumbnail_batch", data) as any
  },
})
