...
export const FilesGetTemporaryLinkInput = z.object({
  path: z.string().optional().describe("The path to the file you want a temporary link to."),
})

export const FilesGetTemporaryLinkOutput = z.object({
  link: z.string().optional().describe("The temporary link which can be used to stream content the file."),
  metadata: z.object({
...
}).describe("metadata: Metadata of the file.\nlink: The temporary link which can be used to stream content the file.\n")

export const filesGetTemporaryLink = pikkuSessionlessFunc({
  ...
  input: FilesGetTemporaryLinkInput,
  output: FilesGetTemporaryLinkOutput,
  func: async ({ dropbox }, data) => {
    return dropbox.call("POST", "/files/get_temporary_link", data) as any
  },
})
