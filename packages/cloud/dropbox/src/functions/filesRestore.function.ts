...
export const FilesRestoreInput = z.object({
  path: z.string().optional().describe("The path to the file you want to restore."),
  rev: z.string().optional().describe("The revision to restore for the file."),
})

export const FilesRestoreOutput = z.object({
...
export const filesRestore = pikkuSessionlessFunc({
  description: "Restore a file to a specific revision.",
  input: FilesRestoreInput,
  output: FilesRestoreOutput,
  func: async ({ dropbox }, data) => {
    return dropbox.call("POST", "/files/restore", data) as any
  },
})
