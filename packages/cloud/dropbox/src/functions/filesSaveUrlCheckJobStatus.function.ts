...
export const FilesSaveUrlCheckJobStatusInput = z.object({
...
export const FilesSaveUrlCheckJobStatusOutput = z.object({
  failed: z.object({
    path: z.object({
    ...
    ".tag": z.enum(["path", "download_failed", "invalid_url", "not_found", "other"]).optional(),
...
export const filesSaveUrlCheckJobStatus = pikkuSessionlessFunc({
  description: "Check the status of a :route:`save_url` job.",
  input: FilesSaveUrlCheckJobStatusInput,
  output: FilesSaveUrlCheckJobStatusOutput,
  func: async ({ dropbox }, data) => {
    return dropbox.call("POST", "/files/save_url/check_job_status", data) as any
  },
})
