...
export const SharingShareFolderInput = z.object({
  ...
  force_async: z.boolean().optional().describe("Whether to force the share to happen asynchronously."),
...
})).optional().describe("A list of `FolderAction`s corresponding to `FolderPermission`s that should appear in the  response's :field:`SharedFolderMetadata.permissions` field describing the actions the  authenticated user can perform on the folder."),
  ...
  path: z.string().optional().describe("The path to the folder to share. If it does not exist, then a new one is created."),
  link_settings: z.object({
  ...
  password: z.object({
    ".tag": z.enum(["remove_password", "set_password", "other"]).optional(),
    set_password: z.string().optional().describe("Set a new password or change an existing password."),
  }).optional().describe("remove_password: Remove the currently set password for the link.\nset_password: Set a new password or change an existing password.\nother: None\n"),
  expiry: z.object({
    set_expiry: z.string().optional().describe("Set a new expiry or change an existing expiry."),
    ".tag": z.enum(["remove_expiry", "set_expiry", "other"]).optional(),
  }).optional().describe("remove_expiry: Remove the currently set expiry for the link.\nset_expiry: Set a new expiry or change an existing expiry.\nother: None\n"),
}).optional().describe("Settings that apply to a link.\naccess_level: The access level on the link for this file. Currently, it only accepts 'viewer' and 'viewer_no_comment'.\naudience: The type of audience on the link for this file.\nexpiry: An expiry timestamp to set on a link.\npassword: The password for the link.\n"),
...
export const SharingShareFolderOutput = z.object({
...
export const sharingShareFolder = pikkuSessionlessFunc({
  ...
  input: SharingShareFolderInput,
  output: SharingShareFolderOutput,
  func: async ({ dropbox }, data) => {
    return dropbox.call("POST", "/sharing/share_folder", data) as any
  },
})
