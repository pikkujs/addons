...
export const FileRequestsUpdateInput = z.object({
  open: z.boolean().optional().describe("Whether to set this file request as open or closed."),
  destination: z.string().optional().describe("The new path of the folder in the Dropbox where uploaded files will be sent. For apps with the app folder permission, this will be relative to the app folder."),
  deadline: z.object({
  ".tag": z.enum(["no_update", "update", "other"]).optional(),
  update: z.object({
...
}).optional().describe("no_update: Do not change the file request's deadline.\nupdate: If :val:`null`, the file request's deadline is cleared.\nother: None\n"),
  id: z.string().optional().describe("The ID of the file request to update."),
  title: z.string().optional().describe("The new title of the file request. Must not be empty."),
})

export const FileRequestsUpdateOutput = z.object({
...
export const fileRequestsUpdate = pikkuSessionlessFunc({
  description: "Update a file request.",
  input: FileRequestsUpdateInput,
  output: FileRequestsUpdateOutput,
  func: async ({ dropbox }, data) => {
    return dropbox.call("POST", "/file_requests/update", data) as any
  },
})
