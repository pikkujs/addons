...
export const SharingListMountableFoldersInput = z.object({
  limit: z.number().optional().describe("The maximum number of results to return per request."),
...
export const SharingListMountableFoldersOutput = z.object({
  cursor: z.string().optional().describe("Present if there are additional shared folders that have not been returned yet. Pass the cursor into the corresponding continue endpoint (either :route:`list_folders/continue` or :route:`list_mountable_folders/continue`) to list additional folders."),
