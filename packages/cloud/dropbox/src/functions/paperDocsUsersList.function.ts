...
export const PaperDocsUsersListInput = z.object({
  ...
  filter_by: z.object({
  ".tag": z.enum(["visited", "shared", "other"]).optional(),
}).optional().describe("visited: all users who have visited the Paper doc.\nshared: All uses who are shared on the Paper doc. This includes all users who have visited the Paper doc as well as those who have not.\nother: None\n"),
})

export const PaperDocsUsersListOutput = z.object({
  cursor: z.object({
...
export const paperDocsUsersList = pikkuSessionlessFunc({
  ...
  input: PaperDocsUsersListInput,
  output: PaperDocsUsersListOutput,
  func: async ({ dropbox }, data) => {
    return dropbox.call("POST", "/paper/docs/users/list", data) as any
  },
})
