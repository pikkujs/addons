...
})).optional().describe("A list of `FileAction`s corresponding to `FilePermission`s that should appear in the  response's :field:`SharedFileMetadata.permissions` field describing the actions the  authenticated user can perform on the file."),
})

export const SharingGetFileMetadataBatchOutput = z.array(z.object({
  result: z.object({
    ...
    ".tag": z.enum(["metadata", "access_error", "other"]).optional(),
    metadata: z.object({
      owner_team: z.object({
