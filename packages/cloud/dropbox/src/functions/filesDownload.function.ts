...
export const FilesDownloadInput = z.object({
  path: z.string().optional().describe("The path of the file to download."),
  rev: z.string().optional().describe("Please specify revision in :field:`path` instead."),
})

export const FilesDownloadOutput = z.object({
...
export const filesDownload = pikkuSessionlessFunc({
  description: "Download a file from a user's Dropbox.",
  input: FilesDownloadInput,
  output: FilesDownloadOutput,
  func: async ({ dropbox }, data) => {
    return dropbox.call("POST", "/files/download", data) as any
  },
})
