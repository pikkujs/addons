...
export const SharingChangeFileMemberAccessInput = z.object({
  member: z.object({
  ...
  file: z.string().optional().describe("File for which we are changing a member's access."),
})

export const SharingChangeFileMemberAccessOutput = z.object({
  member: z.object({
    ".tag": z.enum(["dropbox_id", "email", "other"]).optional(),
    dropbox_id: z.string().optional().describe("Dropbox account, team member, or group ID of member."),
    email: z.string().optional().describe("E-mail address of member."),
...
}).describe("Per-member result for :route:`add_file_member` or :route:`change_file_member_access`.\nmember: One of specified input members.\nresult: The outcome of the action on this member.\n")

export const sharingChangeFileMemberAccess = pikkuSessionlessFunc({
  description: "Identical to update_file_member but with less information returned.",
  input: SharingChangeFileMemberAccessInput,
  output: SharingChangeFileMemberAccessOutput,
  func: async ({ dropbox }, data) => {
    return dropbox.call("POST", "/sharing/change_file_member_access", data) as any
  },
})
