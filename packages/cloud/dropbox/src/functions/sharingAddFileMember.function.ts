...
export const SharingAddFileMemberInput = z.object({
...
})).optional().describe("Members to add. Note that even an email address is given, this may result in a user being directy added to the membership if that email is the user's main account email."),
  quiet: z.boolean().optional().describe("Whether added members should be notified via device notifications of their invitation."),
  add_message_as_comment: z.boolean().optional().describe("If the custom message should be added as a comment on the file."),
  ...
  file: z.string().optional().describe("File to which to add members."),
  custom_message: z.string().optional().describe("Message to send to added members in their invitation."),
})

export const SharingAddFileMemberOutput = z.array(z.object({
  member: z.object({
    ".tag": z.enum(["dropbox_id", "email", "other"]).optional(),
    dropbox_id: z.string().optional().describe("Dropbox account, team member, or group ID of member."),
    email: z.string().optional().describe("E-mail address of member."),
  ...
  }).optional().describe("success: Member was successfully removed from this file. If AccessLevel is given, the member still has access via a parent shared folder.\nmember_error: User was not able to perform this action.\n"),
}))

export const sharingAddFileMember = pikkuSessionlessFunc({
  description: "Adds specified members to a file.",
  input: SharingAddFileMemberInput,
  output: SharingAddFileMemberOutput,
  func: async ({ dropbox }, data) => {
    return dropbox.call("POST", "/sharing/add_file_member", data) as any
  },
})
