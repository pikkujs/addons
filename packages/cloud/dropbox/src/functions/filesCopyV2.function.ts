...
export const FilesCopyV2Input = z.object({
...
export const FilesCopyV2Output = z.object({
  metadata: z.object({
...
}).describe("metadata: Metadata of the relocated object.\n")

export const filesCopyV2 = pikkuSessionlessFunc({
  description: "Copy a file or folder to a different location in the user's Dropbox.\nIf the source path is a folder all its contents will be copied.",
  input: FilesCopyV2Input,
  output: FilesCopyV2Output,
  func: async ({ dropbox }, data) => {
    return dropbox.call("POST", "/files/copy_v2", data) as any
  },
})
