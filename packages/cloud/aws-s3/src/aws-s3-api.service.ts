...
const ROUTES: Record<string, { path: string[], query: string[], headers: string[], errors?: Record<number, string> }> = {
  ...
  "DELETE /folder/{bucket}/{folderKey}": {
    "path": [
      "bucket",
      "folderKey"
    ],
...
export class AwsS3Service {
  ...
  constructor(private creds: { apiKey: string }, variables: TypedVariablesService) {
    this.baseUrl = variables.get('AWS_S3_BASE_URL') as string
  }
  ...
  async call<T>(
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        default: throw new Error(`AWS S3 API error (${response.status}): ${errorText}`)
      }
    }
