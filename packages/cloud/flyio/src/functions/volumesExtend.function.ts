...
export const VolumesExtendInput = z.object({
  app_name: z.string().describe("Fly App Name"),
  volume_id: z.string().describe("Volume ID"),
  size_gb: z.number().int().optional(),
})

export const VolumesExtendOutput = z.object({
...
export const volumesExtend = pikkuSessionlessFunc({
  description: "Extend a volume's size within an app using the details provided in the request body.",
  input: VolumesExtendInput,
  output: VolumesExtendOutput,
  func: async ({ flyio }, data) => {
    return flyio.call('PUT', '/apps/{app_name}/volumes/{volume_id}/extend', data) as any
  },
})
