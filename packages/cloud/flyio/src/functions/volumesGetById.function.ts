...
export const VolumesGetByIdInput = z.object({
  app_name: z.string().describe("Fly App Name"),
  volume_id: z.string().describe("Volume ID"),
})

export const VolumesGetByIdOutput = z.object({
...
export const volumesGetById = pikkuSessionlessFunc({
  description: "Retrieve details about a specific volume by its ID within an app.",
  input: VolumesGetByIdInput,
  output: VolumesGetByIdOutput,
  func: async ({ flyio }, data) => {
    return flyio.call('GET', '/apps/{app_name}/volumes/{volume_id}', data) as any
  },
})
