...
describe('PostHogAnalyticsMapper', () => {
  ...
  test('falls back to the pikku id, so a session stays one person', () => {
  ...
  test('falls back to a minted device id for a visitor with no session', () => {
    const event = new PostHogAnalyticsMapper().toEvent(
      record({ userIdentity: { userId: null, anonymousId: 'device-9' } })
    )

    assert.equal(event?.distinct_id, 'device-9')
  })

  test('drops a record identifying nobody rather than merging everyone', () => {
    const event = new PostHogAnalyticsMapper().toEvent(
